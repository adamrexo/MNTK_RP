...
function showGeneralshopUI(shop_type, race, gender, discount, products1)
	...
	if not wCustomShop and not wAddingItemsToShop and not wGeneralshop and not getElementData(localPlayer, "shop:NoAccess" ) then
		...
		if shop_type==17 then
			...
			local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'Vítejte v '..bizName..' - Dvojitým kliknutím na věc ji koupíte',true,tProducts)
			bCloseStatic = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Zavřít", true, tProducts)
			...
			local colName = guiGridListAddColumn(gProducts,"Název",0.2)
			local colAmount = guiGridListAddColumn(gProducts,"Podrobnosti",0.2)
			local colPrice = guiGridListAddColumn(gProducts,"Cena",0.1)
			local colDesc = guiGridListAddColumn(gProducts,"Popis",0.36)
			--local colDate = guiGridListAddColumn(gProducts,"Published Date",0.15)
			local colProductID = guiGridListAddColumn(gProducts,"ID produktu",0.1)
			local currentCap = 0
			...
			if isClientBizOwner or exports.global:hasItem(localPlayer, 5, getElementDimension(localPlayer)) then
				...
				local bClose = guiCreateButton(0.5, 0.88, 0.49, 0.1, "Zavřít", true, tGoodBye)
				...
				local bClose = guiCreateButton(0.695, 0.88, 0.3, 0.1, "Zavřít", true, tBizManagement)
				guiSetFont(bClose, BizNoteFont2)
				addEventHandler( "onClientGUIClick", bClose, hideGeneralshopUI, false )
			else
				-----------------------------------------CUSTOMER PANEL-----------------------------------------------------------------

				tGoodBye = guiCreateTab ( "Kontaktní informace" , shopTabPanel )

				local lTitle1 = guiCreateLabel(11,19,716,56,(bizName.." - Uvidíme se znova!"),false,tGoodBye)
					--guiLabelSetVerticalAlign(lTitle1[1],"center")
					guiLabelSetHorizontalAlign(lTitle1,"center",false)
					guiSetFont(lTitle1, BizNoteFont)
				-- Fetching info
				...
				local eContent = guiCreateEdit ( 0.2, 0.58, 0.79, 0.1,"obsah", true, tGoodBye)
				...
				addEventHandler( "onClientGUIClick", bSend, function()
					...
					if contactContent and contactContent ~= "" and contactContent ~= "obsah" then
		...
		elseif shop_type==19 then --Faction Drop NPC - Weapons
			...
			fdgw.colName = guiGridListAddColumn(fdgw.gProducts,"Název",0.18)
			fdgw.colPrice = guiGridListAddColumn(fdgw.gProducts,"Cena",0.08)
			fdgw.colDesc = guiGridListAddColumn(fdgw.gProducts,"Popis",0.4)
			fdgw.colQuantity = guiGridListAddColumn(fdgw.gProducts,"Na skladě",0.06)
			fdgw.colRestock = guiGridListAddColumn(fdgw.gProducts,"Doplnění zásob",0.15)
			...
			if canPlayerAdminShop(localPlayer) then
				...
				local bCloseStatic2 = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Zavřít", true, tBizManagement)
			...
			if shop_type == 7 then
				if not exports.global:hasItem(localPlayer, 183) then -- Viozy Membership Card
					outputChatBox("Musíš mít viozy kartičku, přijď až ji budeš mít.", 255, 0, 0)
					hideGeneralshopUI()
					return
				end
			end
			...
			for _, category in ipairs( shop ) do
				...
				local cName = guiGridListAddColumn(grid, "Název", 0.18)
				local cPrice = guiGridListAddColumn(grid, "Cena", 0.08)
				local cDescription = guiGridListAddColumn(grid, "Popis", govOwned and 0.7 or 0.6)
				local cQuantity = false
				if not govOwned then
					cQuantity = guiGridListAddColumn(grid, "Množství", 0.1)
				end
					...
					if checkItemSupplies(shop_type, supplies, item.itemID, item.itemValue, getMetaItemName(item)) or govOwned then
						...
						if not govOwned then
							if bandanas[item.itemID] then
								guiGridListSetItemText(grid, row, cQuantity, supplies["122"] or "Vyprodáno", false, false)
							elseif item.itemID == 16 then
								guiGridListSetItemText(grid, row, cQuantity, supplies["16"] or "Vyprodáno", false, false)
							elseif item.itemID == 114 then
								guiGridListSetItemText(grid, row, cQuantity, supplies["114:" .. vehicle_upgrades[tonumber(item.itemValue)-999][3]] or "Vyprodáno", false, false)
							else
								local supply = supplies[tostring(item.itemID) .. ":" .. (item.itemValue or 1)] or supplies[tostring(item.itemID) .. ":" .. (item.itemValue or 1) .. ":" .. getMetaItemName(item)] or "Vyprodáno"
				...
				local lWelcomeText = guiCreateLabel(0,0.89,0.848,0.1,'Vítejte v '..bizName..' - Dvojitým kliknutím na věc ji koupíte',true,tab)
				guiLabelSetVerticalAlign(lWelcomeText,"center")
				guiLabelSetHorizontalAlign(lWelcomeText,"center",false)
				guiSetFont(lWelcomeText, BizNoteFont18)
				if isClientBizOwner then
				...
				bCloseStatic[_] = guiCreateButton(0.85, 0.90 , 0.15, 0.089, "Zavřít", true, tab)
			...
			if isClientBizOwner or exports.global:hasItem(localPlayer, 5, getElementDimension(localPlayer)) then
				...
				tGoodBye = guiCreateTab ( "Kontaktní informace" , tabpanel )
				...
				local eBargainName = guiCreateEdit ( 0.01, 0.58, 0.19, 0.1,"vaše identita", true, tGoodBye)
				addEventHandler( "onClientGUIClick", eBargainName, function()
					guiSetText(eBargainName, "")
				end, false )
