...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney, CLEANUP)
	...
	--	clearSafe( dbid, true )
		...
		if interiorType == 0 or interiorType == 1 then
			...
			local gov = exports.factions:getFactionFromID(1)
			...
			if interiorStatus.owner == getElementData(thePlayer, "dbid") then
				...
				if givemoney then
					...
					if exports.global:takeMoney(gov, money, true) then
						exports.bank:addBankTransactionLog(-getElementData(gov, "id"), interiorStatus.owner, money, 3 , "Interiér prodán Vládě", getElementData(interiorElement, "name").." (ID: "..getElementData(interiorElement, "dbid")..")" )
					end
				end
			...
			elseif exports.factions:isPlayerInFaction(thePlayer, interiorStatus.faction) then
				...
				if givemoney and faction then
					...
					if exports.global:takeMoney(gov, money, true) then
						exports.bank:addBankTransactionLog(-getElementData(gov, "id"), -interiorStatus.faction, money, 3 , "Interiér prodán Vládě", getElementData(interiorElement, "name").." (ID: "..getElementData(interiorElement, "dbid")..")" )
					end
				end
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	if not exports.global:canPlayerBuyInterior(player) then
		outputChatBox("Máš moc interiérů.", player, 255, 0, 0)
		return false
	end
	...
	if isRentable then
		...
		if result then
			...
			if count ~= 0 then
				outputChatBox("Již pronajímáš nemovitost.", player, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(player, 4, 1) then
		outputChatBox("Nemáš místo na klíče.", player, 255, 0, 0)
		return
	end

	if exports.global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Gratuluji! Zakoupil jsi si tuto nemovitost za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("GOV"), cost )
		elseif (isRentable) then
			outputChatBox("Gratuluji! Pronajímáš tuto nemovitost za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Gratuluji! Zakoupil jsi si tento business za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("GOV"), cost )
		end
		...
		outputChatBox("Nemáš dostatek peněz.", player, 255, 194, 14)
	end
end
...
function buypropertyForFaction(interior, cost, isHouse, furniture, factionName)
	...
	if not exports.global:takeMoney(theFaction, cost) then
		outputChatBox("Není ve frakci dost peněz.", source, 255, 0, 0)
		return
	end
	local gov = getTeamFromName("GOV")
	local intName = getElementData(interior,"name")
	local intId = getElementData(interior, "dbid")
	exports.global:giveMoney( gov, cost )
	exports.bank:addBankTransactionLog(-factionId, -(getElementData(gov, "id")), cost, 3 , "Koupě Nemovistosti", intName.." (ID: "..intId..")" )
	...
	outputChatBox("Gratuluji! Zakoupil jsi si tuto nemovitost for your faction '"..factionName.."' za $" .. exports.global:formatMoney(cost) .. ".", source, 255, 194, 14)
...
function buyInteriorCash(pickup, cost, isHouse, isRentable, furniture)
	if not exports.global:canPlayerBuyInterior(client) then
		outputChatBox("Máš maximum nemovitostí, použij výhodu pod klávesou F10 -> Donátor Výhody.", client, 255, 0, 0)
		return
	end
	...
	if isRentable then
		...
		if result then
			local count = tonumber(result['cntval'])
			if count ~= 0 then
				outputChatBox("Již pronajímáš nemovitost.", client, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(client, 4, 1) then
		outputChatBox("Nemáš místo na klíče.", client, 255, 0, 0)
		return
	end
	...
	if exports.global:takeMoney(client, cost) then
		...
		local gov = getTeamFromName("GOV")
		if (isHouse) then
			outputChatBox("Gratuluji! You have just bought this house for $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
			exports.global:giveMoney( gov, cost )
			exports.bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 5 , "Interior Purchase", intName.." (ID: "..pickupid..")" )
		elseif (isRentable) then
			outputChatBox("Gratuluji! You are now renting this property for $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
		else
			outputChatBox("Gratuluji! You have just bought this business for $" .. exports.global:formatMoney(cost) .. ".", client, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("GOV"), cost )
			exports.bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 5 , "Interior Purchase", intName.." (ID: "..pickupid..")" )
		end
		...
		outputChatBox("Nemáš dostatek peněz.", client, 255, 194, 14)
	end
end
...
function buyInteriorBank(pickup, cost, isHouse, isRentable, furniture)
	...
	if not exports.bank:takeBankMoney(client, cost) then
		outputChatBox( "Nemáš dostatek peněz.", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		local gov = getTeamFromName("GOV")
...
function buyInteriorToken(pickup, furniture)
	if not exports.global:canPlayerBuyInterior(client) then
		outputChatBox("Máš maximum nemovistostí, použij výhodu pod klávesou F10 -> Donátor Výhody.", client, 255, 0, 0)
		return
	end
	...
	if not exports.global:takeItem(client, 262) then
		outputChatBox( "Nemáš token pro koupi.", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		outputChatBox("Gratuluji! Zakoupil jsi si dům za token. Tento dům nemá žádnou hodnotu a nemúžeš ho prodat kamarádovi.", client, 255, 194, 14)
...
function moveSafe ( thePlayer, commandName )
	...
	if (dbid < 19000 and (exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.global:hasItem(thePlayer, 3, dbid - 20000)) then
		if getPedContactElement(thePlayer) == safeTable[dbid] or getPedContactElement(thePlayer) == exports.vehicle:getSafe(dbid-20000) then
            outputChatBox("Pohni se na novou pozici.", thePlayer, 255, 0, 0)
        else
			z = z - 0.5
			rotz = rotz + 180
...
--[[function lockPicknuti(player, checkdistance)
	if (player) then
		source = player
	end
	local itemValue = nil
	...
	if found and itemValue then
		...
		if getElementData(interiorElement, "keypad_lock") then
			if not (exports.integration:isPlayerTrialAdmin(source) and getElementData(source, "duty_admin") == 1) then
				exports.hud:sendBottomNotification(source, "Keypad", "Tyto dveře nemají klíčovou dírku, použijte Keypad.")
				return false
			end
		end
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") odemkl interiér ID #"..itemValue.." bez klíčů.")
					exports.global:sendLocalText(source, " * Dveře jsou odemčené. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "unlock without key", source)
				end
			else
				triggerEvent('sendAme', source, "použil"..pohlavi.." klíč pro odemčení dveří.")
			end
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") zamkl interiér ID #"..itemValue.." bez klíčů")
					exports.global:sendLocalText(source, " * Dveře jsou zamčené. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "lock without key", source)
				end
			else
				triggerEvent('sendAme', source, "použil"..pohlavi.." klíč pro zamčení dveří.")
			end
...
addEvent( "lockPicknuti",false )
addEventHandler( "lockPicknuti", root, lockPicknuti)]]--
...
function lockUnlockHouseEvent(player, checkdistance)
	...
	if found and itemValue or (found and itemValue and lockpick==1) then
		local dbid, entrance, exit, interiorType, interiorElement = findProperty( source, itemValue )
		local playSoundAt = getElementType(found) == "elevator" and found or interiorElement
	...
	function( id, usingKeypad, playSoundAt )
		...
		if id and tonumber(id) and (hasKey1 or usingKeypad) then
			...
			if not usingKeypad then
				...
				if locked == 0 then
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") odemknul interiér ID #"..id.." bez klíče.")
							exports.global:sendLocalText(source, " * Dveře jsou odemčené. *", 255, 51, 102, 30, {}, true)
					...
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") zamknul interiér ID #"..id.." bez klíče.")
							exports.global:sendLocalText(source, " * Dveře jsou zamčené. *", 255, 51, 102, 30, {}, true)
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "odemkl"..pohlavi.." dveře.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "zamknul"..pohlavi.." dveře.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "LOCK INTERIOR")
				end
...
function client_requestHUDinfo()
	...
	triggerClientEvent(client, "displayInteriorName", source, interiorName or "Výtah", interiorOwnerName, interiorType or 2, interiorCost or 0, interiorID or -1, interiorBizNote)
...
function timedInteriorView(houseID)
	...
	if entrance then
		...
		outputChatBox( "Nyní si prohlížíš interiér. Nemůžeš vyhazovat předměty. Můžeš zrušit prohlídku opuštěním interiéru, nebo vyčkej 60 sekund.", client, 0, 255, 0)
		...
		if getElementDimension(client) > 0 then
			...
			outputChatBox( "Pokud si chceš prohlídnout dům z více uhlů, použij /freecam pro lepší výhled.", client, 0, 255, 0 )
		end
		...
		outputChatBox( "Chybná nemovitost.", client, 255, 0, 0 )
	end
end
...
function endTimedInteriorView(thePlayer, changedCharacter )
	...
	if info and pos then
		...
		if entrance then
			...
			outputChatBox( "Skončila ti prohlídka.", thePlayer, 0, 255, 0 )
		else
			outputChatBox( "Chybná nemovitost.", thePlayer, 255, 0, 0 )
		end
	end
...
function vykrast(thePlayer)
local x,y,z = getElementPosition(thePlayer)
if exports["factions"]:getPlayersInFaction(2) then
	local hraci = exports["factions"]:getPlayersInFaction(2)
		for key,value in ipairs(hraci) do
			setTimer(function()
  			outputChatBox("[OHLÁŠENÍ] Byla zjištěna krádež domu!", value, 0, 150, 255, true)
			outputChatBox("[OHLÁŠENÍ] Nevíme přesnou lokaci ale máte bližší lokaci na mapě! (BÍLÝ ČTVEREČEK)", value, 0, 150, 255, true)
			blipos = createBlip( x, y-5, z, 1, 0, 0, 0, 255 )
			end, 25000, 1)
		end
end
	if exports["global"]:hasItem(thePlayer, 322) then
		outputChatBox("Začal si vykrádat, pozor na polici!", thePlayer, 255, 255, 255, true)
		triggerClientEvent(thePlayer, "vykrast:progress", thePlayer)
		exports["global"]:takeItem(thePlayer, 322)
	else
		outputChatBox("Nemáš páčidlo! to se dá zakoupit všude možně!", thePlayer, 255, 0, 0, true)
	end

	setTimer(function()
	destroyElement(blipos)
	end, 30000, 1)
end
addEvent("vykrast:start", true)
addEventHandler("vykrast:start", root, vykrast)


function zacitvykradat(thePlayer, idInterier)
	local dbid, entrance, exit, interiorType, idInterier = findProperty( thePlayer, idInterier )
 	setPlayerInsideInterior(idInterier, thePlayer) 
	outputChatBox("Máte 20 sekund na to sebrat co nejvíce věcí!", thePlayer, 0, 255, 0, true)	
	number = 0
	outputChatBox("Proces vykrádání:", thePlayer, 255, 255, 255, true)
	timer2 = setTimer(function ()
		number = number + 10
		outputChatBox(number.."%/200%", thePlayer, 0, 255, 0, true)
		if number == 200 then
	...
	setTimer(function()
	outputChatBox("Tvůj čas vypršel, nyní bych raději zdrhnul rychle poldům", thePlayer, 0, 150, 255, true)
	setPlayerInsideInterior(idInterier, thePlayer)
	setElementFrozen(thePlayer, false)
	exports["global"]:giveMoney(thePlayer, math.random(5000,15000))
	end, 20000, 1)
end
addEvent("vykrast:zacit", true)
addEventHandler("vykrast:zacit", root, zacitvykradat)


function editInterieru(thePlayer)
	if getElementDimension(thePlayer) == 0 then
		return false
	end
	local result = mysql:query( "SELECT * FROM `interior_nakupy` WHERE `hrac`='"..getPlayerName(thePlayer).."'")
    if result then
        t = { }
        while true do
            local row = mysql:fetch_assoc( result )
            if row then
                table.insert(t, { row.id, row.nakup })
            else
                break
            end
        end
	...
	triggerClientEvent ( thePlayer, "interior:inv", thePlayer )
	setTimer(function()
		triggerClientEvent ( thePlayer, "interior:invnacist", thePlayer, t )
	end, 100, 1)
end
addCommandHandler("intedit", editInterieru)


function vytvoritDoplnekInt(thePlayer, model, nazev, id)
	if model and nazev then
		local data = mysql:query_fetch_assoc( "SELECT * FROM `interior_doplnky`")
		local hrac = getPlayerName(thePlayer)
		local interier = getElementInterior(thePlayer)
		local dimenze = getElementDimension(thePlayer)
		local doplnek = nazev
		local mod = model
		local x,y,z = getElementPosition(thePlayer)
		local rx,rz,ry = 0,0,0
		local insert = mysql:query("INSERT INTO `interior_doplnky` SET `hrac`='"..hrac.."', `interier`='"..interier.."', `dimenze`='"..dimenze.."', `doplnek`='"..doplnek.."', `model`='"..mod.."', `x`='"..x.."', `y`='"..y.."', `z`='"..z.."', `rx`='"..rx.."', `rz`='"..rz.."', `ry`='"..ry.."'")
		local delete = mysql:query("DELETE FROM `interior_nakupy` WHERE `id`='"..id.."'")
		if insert then
			executeCommandHandler("intedit", thePlayer)
			kontrolyDoplnkuInterieru()
			setElementPosition(thePlayer, x+2,y,z)
		else
			outputChatBox("Nastala chyba, kontaktuj správce serveru ", thePlayer, 255, 0, 0, true)
		end
	end
end
addEvent("interior:vytvorit", true)
addEventHandler("interior:vytvorit", root, vytvoritDoplnekInt)
local objektCont = createElement("objektCont")
local prvobjektCont = createElement("prvobjektCont")

function kontrolyDoplnkuInterieru()
    local players = getElementsByType ( "player" )
    for theKey,thePlayer in ipairs(players) do
    local result = mysql:query( "SELECT * FROM `interior_doplnky`")
    if result then
        while true do
            local row = mysql:fetch_assoc( result )
            if row then
				if isElement(objektCont) then
					destroyElement(objektCont)
					destroyElement(prvobjektCont)
					objektCont = createElement("objektCont")
					prvobjektCont = createElement("prvobjektCont")
				end
				if isElement(pervObj) then
					destroyElement(pervObj)
				end
				setTimer(function()
                    objekta = createObject(row.model, row.x, row.y, row.z, row.rx, row.ry, row.rz)
                    setElementDimension(objekta, row.dimenze)
                    setElementInterior(objekta, row.interier)
					setElementData(objekta, "objekt_id", row.id)
					setElementParent(objekta, objektCont) 
				end, 1000, 1)
...
function aktualizovatDoplnkyINT(thePlayer, id, nazev, model, x, y, z, rx, ry, rz, dimenze, interier)
	if id and nazev then
	if isElement(objekta) then
	setElementModel(objekta, model)
	setElementPosition(objekta, x,y,z)
	setElementRotation(objekta, rx,ry,rz)
	setElementDimension(objekta, dimenze)
	end
	end
end
...
function ulozitdoplnekInterieru(thePlayer, nazev, model, x,y,z, rx,ry,rz, id, dim, int)	
	local update = mysql:query("UPDATE `interior_doplnky` SET `x`='"..x.."', `y`='"..y.."', `z`='"..z.."', `rx`='"..rx.."', `ry`='"..ry.."',`rz`='"..rz.."' WHERE `id`='"..id.."'")
	if update then
		if id then
			kontrolyDoplnkuInterieru()
		end
...
addEvent("interior:ulozit", true)
addEventHandler("interior:ulozit", root, ulozitdoplnekInterieru)


function poziceNastavitRotace(thePlayer, nazev, model, x,y,z, rx,ry,rz, id, dim, int)
	local result = mysql:query_fetch_assoc("SELECT * FROM `interior_doplnky` WHERE `id`='"..id.."'")
	if result then
		local update = mysql:query("UPDATE `interior_doplnky` SET `x`='"..x.."', `y`='"..y.."', `z`='"..z.."', `rx`='"..rx.."', `ry`='"..ry.."',`rz`='"..rz.."' WHERE `id`='"..id.."'")
		aktualizovatDoplnkyINT(thePlayer, id, nazev, model, x, y, z, rx, ry, rz, dim, int)			
    end
end
addEvent("interior:poziceRotace", true)
addEventHandler("interior:poziceRotace", root, poziceNastavitRotace)

function necoUdelatsInt(thePlayer, id)
	local datas = mysql:query_fetch_assoc("SELECT * FROM `interior_doplnky` WHERE `id`='"..id.."' AND `hrac`='"..getPlayerName(thePlayer).."'")
	if datas then
		--showChat ( false ) 
		triggerClientEvent(thePlayer, "interior:pozicovani", thePlayer, datas["doplnek"], datas["model"], datas["x"], datas["y"], datas["z"], datas["rx"], datas["ry"], datas["rz"], id, datas["dimenze"], datas["interier"] )
	end
	end
addEvent("interior:otevriklik", true)
addEventHandler("interior:otevriklik", root, necoUdelatsInt)



function zpetDoInvINT(thePlayer, nazev, x, y, z, id, dim, int)

	local insert = mysql:query("INSERT INTO `interior_nakupy` SET `hrac`='"..getPlayerName(thePlayer).."', `nakup`='"..nazev.."'")
	local delete = mysql:query("DELETE FROM `interior_doplnky` WHERE `id`='"..id.."'")
	if insert and delete then
		editInterieru(thePlayer)
		kontrolyDoplnkuInterieru()
	else
		outputChatBox("Vyskytla chyba, kontaktuj správce serveru", thePlayer, 255)
	end
end
addEvent("interior:zpetdoinv", true)
addEventHandler("interior:zpetdoinv", root, zpetDoInvINT)


function objektyVedleTebe(thePlayer, nazev, model, x,y,z)
	if nazev then
		local datas = mysql:query_fetch_assoc("SELECT * FROM `interior_doplnky` WHERE `doplnek`='"..nazev.."'")
		if datas then
			local pars = mysql:query_fetch_assoc("SELECT * FROM `interior_doplnky` WHERE `model`='"..model.."' AND `x`='"..x.."' AND `y`='"..y.."' AND `z`='"..z.."'")
			if pars then
				local id = pars["id"]
				outputChatBox("#ffffffID: #7aff69"..id.."#ffffff | Název: #7aff69"..nazev, thePlayer, 255, 255, 255, true)
			end
		end
	end
end
addEvent("interior:objektyvedle", true)
addEventHandler("interior:objektyvedle", root, objektyVedleTebe)

 --[[
function datvsem()


		mysql:query("UPDATE `interiors` SET `interiorx`='1317.94', `interiory`='1482.81', `interiorz`='16.025'")

end
addCommandHandler("datvsude", datvsem)]]--