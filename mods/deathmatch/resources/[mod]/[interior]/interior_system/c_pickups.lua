...
function renderInteriorName()
    if hitPickup and isElement(hitPickup) then
        ...
        makeFonts()
        local intInst = "Stiskni F pro vstup"
        ...
        local img_t = scrHeight-img_h-55
        ...
        if isLastSourceInterior then
            ...
            if canPlayerKnowInteriorOwner(theInterior) then -- House or Biz
                ...
                if intStatus.owner > 0 then
                    ...
                    if intType == 3 then
                        intOwner = "Pronájem od "..(ownerName or "..Loading..")
                        intInst = "Stiskni F pro vstup"
                    elseif intType ~= 2 then
                        intOwner = "Owned by "..(ownerName or "..Načítání..")
                        intInst = "Stiskni F pro vstup"
                    end
                elseif intStatus.faction > 0 then
                    local ownerName = exports.cache:getFactionNameFromId(intStatus.faction)
                    if intType ~= 2 then
                        intOwner = "Vlastník: "..(ownerName or "..Načítání..")
                        intInst = "Stiskni F pro vstup"
                    end
                else
                    if intType == 2 then
                        intOwner = "Bez Vlastníka"
                        intInst = "Stiskni F pro vstup"
                    elseif intType == 3 then
                        local intPrice = exports.global:formatMoney(intStatus.cost)
                        intOwner = "Na Prodej: $"..intPrice..""
                        intInst = "Stiskni F pro pronájem"
                    else
                        local intPrice = exports.global:formatMoney(intStatus.cost)
                        intOwner = "Na Prodej: $"..intPrice..""
                        intInst = "Stiskni F pro platbu"
