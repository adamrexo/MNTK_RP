...
local function load( res )
	...
	if result and num_affected_rows > 0 then
		...
		outputDebugString( "[INTERIOR] Načítám "..num_affected_rows.." interiérů. Konec za "..exports.global:formatMoney( (load_speed*num_affected_rows)/1000/load_speed_multipler ).." sekund(s)" )
		triggerLatentClientEvent( 'hud:loading', resourceRoot, 'Načítání Interiérů', { max=total, cur=0 } )
	end
end
...
function resumeThreads()
	for i, co in ipairs( threads ) do
		...
		if percent ~= new_perc then
			percent = new_perc
			triggerLatentClientEvent( 'hud:loading', resourceRoot, 'Načítání Interiérů', { max=total, cur=loaded } )
		end
	...
	if #threads <= 0 then
		...
		triggerLatentClientEvent( 'hud:loading', resourceRoot, 'Načítání Interiérů', { max=total, cur=total } )
