...
function open_log_reg_pannel()
	if not isElement ( panel.login.main ) then
		...
		panel.login.logo = guiCreateStaticImage( (sw-logoSize[1])/2, (sh-logoSize[2])/2 , logoSize[1], logoSize[2], "/login-panel/MNTK_logo1.png", false )
...
function displayRegisterConpleteText(username)
    ...
    local temp = ""
    GUIEditor.label[1] = guiCreateLabel(8, 50, 339, 121+extend, "Váš účet pro MNTK RP na jmeno '"..username.."' je připraven!\n nyní se můžete přihlásit a užít si naši hru \n\nS pozdravem \nVývojový tým MNTK", false, GUIEditor.window[1])
    guiLabelSetHorizontalAlign(GUIEditor.label[1], "left", true)

    GUIEditor.button[1] = guiCreateButton(10, 153+extend, 337, 26, "Přihlásit se", false, GUIEditor.window[1])
