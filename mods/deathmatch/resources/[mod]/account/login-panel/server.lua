	...
	dbQuery(function(qh, username, password, confirmPassword, email, client)
		...
		if result then
			if #result == 0 then
				...
				preparedQuery3 = "INSERT INTO `accounts` SET `username`=?, `password`=?, `email`=?, `registerdate`=NOW(), `ip`=?, `activated`='2' "
				...
				if userid then
					...
					dbQuery(function(qh, client)
						...
						if result and #result == 1 then
							if dbExec(exports.mysql:getConn("mta"), "INSERT INTO account_details SET `account_id`=?, `mtaserial`=?, `appstate`=?", result[1].id, mtaSerial, 3) then
								iprint("Account "..username.." created")
							else
