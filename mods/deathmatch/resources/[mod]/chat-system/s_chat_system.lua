...
local factionsThatCanUseDepartmentRadio = { 1, 2, 3, 4, 10, 13, 20, 40 }
local factionsThatCanUseMedicalRadio = { 3, 4 }

local emoce = { ":D", ":-D", "D:", "XD", "Xd", "xd", "xD", ":)", ":-)", ":(", ":-(", ":c", ":C", ";(", ";-(", ":P", ":p", ":-P", ":-p", "<>", "||", ":3", ":S", ":s", ";)", "<3", ":*", ":O", ":/" }

local emoceakce = {
	{	"se zasmála",			"se zasmál"			},
	{	"se zasmála",			"se zasmál"			},
	{	"se zatvářila vyděšeně",			"se zatvářil vyděšeně"			},
	{	"se zasmála jako blázen",			"se zasmál jako blázen"			},
	{	"se zasmála jako blázen",			"se zasmál jako blázen"			},
	{	"se zasmála jako blázen",			"se zasmál jako blázen"			},
	{	"se zasmála jako blázen",			"se zasmál jako blázen"			},
	{	"se usmála",			"se usmál"			},
	{	"se usmála",			"se usmál"			},
	{	"se zamračila",			"se zamračil"		},
	{	"se zamračila",			"se zamračil"		},
	{	"se zatvářila smutně",			"se zatvářil smutně"		},
	{	"se zatvářila smutně",			"se zatvářil smutně"		},
	{	"se rozbrečela",		"se rozbrečel"		},
	{	"se rozbrečela",		"se rozbrečel"		},
	{	"vyplázla jazyk",		"vyplázl jazyk"		},
	{	"vyplázla jazyk",		"vyplázl jazyk"		},
	{	"vyplázla jazyk",		"vyplázl jazyk"		},
	{	"vyplázla jazyk",		"vyplázl jazyk"		},
	{	"zakroutila hlavou",	"zakroutil hlavou"	},
	{	"kývla hlavou",			"kývl hlavou"		},
	{	"udělala roztomilý obličej",			"udělal roztomilý obličej"		},
	{	"udělala roztomilý obličej",			"udělal roztomilý obličej"		},
	{	"udělala roztomilý obličej",			"udělal roztomilý obličej"		},
	{	"mrknula",			"mrknul"		},
	{	"dala ruce do tvaru srdíčka",			"dal ruce do tvaru srdíčka"		},
	{	"poslala pusu",			"poslal pusu"		},
	{	"se zatvářila překvapeně",			"se zatvářil překvapeně"		},
	{	"se zatvářila poklesle",			"se zatvářil poklesle"		},
}
...
function localIC(source, message, language)
	...
	local maskaID = getElementData(source, "maskaID")
	
	if getElementData(source, "maska") then
	    playerName = "Neznámá Osoba ("..maskaID..")"
	end
	...
	local canemoce = true
	for k, v in ipairs(emoce)do
		if(string.find(message, v, 1, true))then
			message = message:gsub(v:gsub("(%W)","%%%1"), '')
			if canemoce then
				if(getElementData(source, "gender") == 1)then
					exports.global:sendLocalMeAction(source, emoceakce[k][1]..".")
				else
					exports.global:sendLocalMeAction(source, emoceakce[k][2]..".")
				end
				canemoce = false
			end
		end
	...
	if playerVehicle then
		if (exports.vehicle:isVehicleWindowUp(playerVehicle)) then
			...
			outputChatBox( "[" .. languagename .. "] " .. playerName .. " ((Vozidlo)) říká: " .. message, source, unpack(color))
		else
			outputChatBox( "[" .. languagename .. "] " .. playerName .. " říká: " .. message, source, unpack(color))
		end
	else
			...
			if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) then
				...
				if not (isPedDead(nearbyPlayer)) and (logged==1) and (nearbyPlayer~=source) then
						...
						if (exports.vehicle:isVehicleWindowUp(pveh)) then
							for i = 0, getVehicleMaxPassengers(pveh) do
								...
								if (lp) and (lp~=source) then
									...
									outputChatBox("[" .. languagename .. "] " .. playerName .. " ((Vozidlo)) říká: " .. message2, lp, unpack(color))
					...
					if nbpveh and exports.vehicle:isVehicleWindowUp(nbpveh) == true then
						...
						outputChatBox("[" .. languagename .. "] " .. playerName .. " říká: " .. message2, nearbyPlayer, unpack(color))]]
...
function meEmote(source, cmd, ...)
	...
	if logged == 1 then
		...
		if not (...) then
			outputChatBox("Použití: /me [Action]", source, 255, 194, 14)
		else
...
function outputChatBoxCar( vehicle, target, text1, text2, color )
	if vehicle and exports.vehicle:isVehicleWindowUp( vehicle ) then
		if getPedOccupiedVehicle( target ) == vehicle then
			outputChatBox( text1 .. " ((Vozidlo))" .. text2, target, unpack(color))
			return true
		else
			return false
		end
...
function radio(source, radioID, message, cmd)
	...
	if jailed > 0 then
		outputChatBox('Ve vězení nemůžete používat rádio.', source, 255, 0, 0)
		return
	end
	...
	if isPedDead(source) then
		outputChatBox(" Když jste mrtvý, nemůžete používat rádio.", source, 255, 0, 0)
		return
	end
	...
	if hasRadio or getElementType(source) == "ped" or radioID == -2 then
		...
		if theChannel == 1 or theChannel == 0 then
			outputChatBox("Rádio nejprve nalaďte pomocí /tuneradio [channel]", source, 255, 194, 14)
		elseif not hasChannelAccess(source, theChannel) then
			outputChatBox("K tomuto kanálu nemáte přístup. Přelaďte prosím své rádio.", source, 255, 194, 14)
		elseif (theChannel > 1 or radioID < 0) and itemValue > 0 then
			...
			if channelName == "COUNTY MED" then
				outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. message, {60,179,113})
			elseif channelName == "DEPARTMENT" then
				outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. message, {r,162,b})
			else
				if cmd == "rlow" then
					outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " šeptá: " .. message, {r,g,b})
					triggerEvent("sendAme", source, "šeptá ve vysílačce.")
				else
					outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. message, {r,g,b})
				end
			end
			...
			for key, value in ipairs(affectedElements) do
				...
				if value ~= source then
					...
					if channelName == "COUNTY MED" then
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. trunklateText( value, message2 ), {60,179,113} )
					elseif channelName == "DEPARTMENT" then
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. trunklateText( value, message2 ), {r,162,b} )
					else
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. languagename .. "] [" .. channelName .. "] " .. username, " říká: " .. trunklateText( value, message2 ), {r,g,b} )
					end
						...
						for k, v in ipairs(exports.global:getNearbyElements(value, "player",7)) do
							...
							if (logged2==1) then
								...
								if not found then
									...
									local text1 = "[" .. languagename .. "] " .. getPlayerName(value) .. " vysílačka"
			...
			for key, value in ipairs(getElementsByType("player")) do
				if cmd == "rlow" and getElementDistance(source, value) < 3 or getElementDistance(source, value) < 10 then
					if (value~=source) then
						...
						if cmd == "rlow" then
							text2 = " šeptá: " .. trunklateText( value, message2 )
						else
							text2 = " říká: " .. trunklateText( value, message2 )
						end
			...
			outputChatBox("Tvoje vysílačka je vypnutá. ((/toggleradio))", source, 255, 0, 0)
		end
	else
		outputChatBox("Nemáš vysílačku.", source, 255, 0, 0)
	end
end
...
function msgRadio(thePlayer, commandName, ...)
	...
	if (...) then
		...
		outputChatBox("Použití: /" .. commandName .. " [Message]", thePlayer, 255, 194, 14)
	end
end
...
function govAnnouncement(thePlayer, commandName, ...)
	...
	if faction > 0 then
		...
		if (fType == 2 or fType == 3 or fType == 4) then
			...
			if #message == 0 then
				outputChatBox("Použití: /" .. commandName .. " [Zpráva]", thePlayer, 255, 194, 14)
				return false
			end

			if not factionLeader then
				outputChatBox("Nemáš právomoce na tento příkaz. (Přidávání Členů)", thePlayer, 255, 0, 0)
			else
				...
				for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
					...
					if (logged==1) then
						outputChatBox(">> Vládní oznámení od " .. factionRankTitle .. " " .. getPlayerName(thePlayer), value, 0, 183, 239)
...
function adminGovAnnouncement(thePlayer, commandName, ...)
	if getElementData(thePlayer, "duty_admin") == 1 then
		...
		for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
			...
			if (logged==1) then
				outputChatBox(">> Vládní Oznámení: ", value, 0, 183, 239)
				outputChatBox(message, value, 0, 183, 239)
			end
		end
	end
end
addCommandHandler("admingov", adminGovAnnouncement)
...
function playerToggleDonatorChat(thePlayer, commandName)
	...
	if (logged==1 and hasPerk) then
		...
		if (tonumber(perkValue)==1) then
			outputChatBox("Nyní jste skryli Donator Chat.", thePlayer, 255, 194, 14)
			exports.donators:updatePerkValue(thePlayer, 9, 0)
		else
			outputChatBox("Nyní jste povolili chat Donator.", thePlayer, 255, 194, 14)
	...
	if hasDonChat or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		if not (...) then
			...
			if (tonumber(togDonChatState) == 0) then
				outputChatBox("[Donator] Odesíláte zprávu, zatímco je váš kanál chatu dárce vypnutý.", thePlayer, 200, 200, 200)
			end
...
function airradio(thePlayer, commandName, ...)
	...
	if playersInAir then
		...
		if found then
			...
			outputChatBox("Nebyl jsi schopen mluvit přes vzdušnou frekvenci.", thePlayer, 255, 0, 0)
		end
	end
end
	...
	if (logged==1) then
		if not (...) then
			...
			if (oocEnabled==0) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerScripter(thePlayer) then
				outputChatBox("OOC Chat je momentálně deaktivován.", thePlayer, 255, 0, 0)
			elseif (muted==1) then
				outputChatBox("Aktuálně jste z OOC chatu ztlumeni.", thePlayer, 255, 0, 0)
			else
				local affectedElements = { }
					...
					if (logged==1) and (targetOOCEnabled==1) then
						...
						if exports.integration:isPlayerTrialAdmin(thePlayer) then
							...
							if getElementData(thePlayer, "hiddenadmin") then
								outputChatBox("(( "..exports.global:getPlayerFullIdentity(thePlayer)..": " .. message .. " ))", arrayPlayer, 255, 172, 28)
							else
								outputChatBox("(( "..exports.global:getPlayerFullIdentity(thePlayer)..": " .. message .. " ))", arrayPlayer, 255, 172, 28)
							end
                        else
...
function playerToggleOOC(thePlayer, commandName)
	...
	if (logged==1) then
		...
		if (playerOOCEnabled==1) then
			outputChatBox("Nyní jste skryli globální chat OOC.", thePlayer, 255, 194, 14)
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "globalooc", 0, false)
		else
			outputChatBox("Nyní jste povolili globální chat OOC.", thePlayer, 255, 194, 14)
...
local advertisementMessages = { "samp", "SA-MP", "Kye", "shodown", "Vedic", "vedic","ventro","Ventro", "server", "sincityrp", "ls-rp", "sincity", "tri0n3", "www.", ".com", "co.cc", ".net", ".co.uk", "everlast", "neverlast", "www.everlastgaming.com", "trueliferp", "truelife", "mtarp", "mta:rp", "mta-rp", "skimo", "crystal", "slovakrp", "slovak"}
...
function scripterChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerScripter(thePlayer))  then
        if not (...) then
            outputChatBox("Použití: /ss [Message]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
...
function vctChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerVCTMember(thePlayer))  then
        if not (...) then
            outputChatBox("Použití: /v [Message]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function mappingTeamChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerMappingTeamMember(thePlayer))  then
        if not (...) then
            outputChatBox("Použití: /mt [Message]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function fmtChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerFMTMember(thePlayer))  then
        if not (...) then
            outputChatBox("Použití: /fmt [Message]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function ignoreOnePlayer(thePlayer, commandName, targetPlayerNick)
	...
	if (logged==1) then
		if not (targetPlayerNick) then
			outputChatBox("Použití: /" .. commandName .. " [Player Partial Nick / ID]", thePlayer, 255, 194, 14)
		else
			...
			if exports.integration:isPlayerStaff(targetPlayer) then
				outputChatBox("Zaměstnanci nesmíte Adminstrátory.", thePlayer, 255, 0, 0)
				return
			end
			...
			for k, v in ipairs(ignoreList[thePlayer] or {}) do
				if v == targetPlayer then
					...
					outputChatBox("Už neignoruješ šepot od " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
					existed = true
					break
				end
			end
			if not existed then
				...
				outputChatBox("Ignorujete zprávy od " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
				outputChatBox("Dejte /ignorelist pro úplný seznam hráčů, které ignorujete.", thePlayer, 0, 255, 0)
			end
		end
	end
end
...
function checkifiamfucked(thePlayer, commandName)
	...
	outputChatBox("    -- MOMENTÁLNĚ IGNOROVÁNÍ --", thePlayer, 2, 172, 19)
	for k, v in ipairs(ignoreList[thePlayer] or {}) do
...
function pmPlayer(thePlayer, commandName, who, ...)
	...
	if tostring(commandName):lower() == "quickreply" and who then
		...
		if not target or not isElement(target) or not (getElementType(target) == "player") or not (getElementData(target, "loggedin") == 1) then
			outputChatBox("Nikdo vám nepsal PM.", thePlayer, 200,200,200)
			return false
		end
		...
		if not (who) or not (...) then
			outputChatBox("Použití: /" .. commandName .. " [Jméno/ID] [Zpráva]", thePlayer, 255, 194, 14)
			outputChatBox("Použij 'U' pro rychlou odpověď.", thePlayer)
			return false
		end
		message = table.concat({...}, " ")
	end
	...
	if who and message then
		...
		if (targetPlayer) then
			if targetPlayer == thePlayer then
				outputChatBox("Nepiš PM sobě.. :D", thePlayer, 255, 0, 0)
				return false
			end

			if getElementData(targetPlayer, "loggedin") ~= 1 then
				outputChatBox("Hráč není přihlášen.", thePlayer, 255, 255, 0)
				return false
			end
			...
			if targetPmPerk and tonumber(targetPmState) == 1 then
				if exports.global:isStaffOnDuty(thePlayer) or (getElementData(thePlayer, "reportadmin") == targetPlayer) then
					...
					if are_they_friends and allow_friends_pm then
						...
						outputChatBox("Hráč má vypnuté příchozí zprávy.", thePlayer, 255, 255, 0)
						return false
					end
				end
			end
			...
			for k, v in ipairs(ignoreList[thePlayer] or {}) do
				if v == targetPlayer then
					outputChatBox('Nyní ignoruješ ' .. targetPlayerName .. '. Odeber si ho z ignor listu.', thePlayer, 255, 0, 0)
					return false
				end
			end
			for k, v in ipairs(ignoreList[targetPlayer] or {}) do
				if v == thePlayer then
					outputChatBox(targetPlayerName .. ' od vás ignoruje PM.', thePlayer, 255, 0, 0)
					return false
				end
			end
			...
			for k,v in ipairs(advertisementMessages) do
				...
				if (found) or (found2) or (string.lower(message)==tostring(v)) then
					exports.global:sendMessageToAdmins("AdminWarn: " .. tostring(playerName) .. " odeslal možnou reklamu v PM hráči " .. tostring(targetPlayerName) .. ".")
					exports.global:sendMessageToAdmins("AdminWarn: Zpráva: " .. tostring(message))
					break
				end
			end

			local mojeJmeno = " (" .. getElementData(thePlayer, "account:username") ..") "
			local targetJmeno = " (" .. getElementData(targetPlayer, "account:username") ..") "
			...
			if exports.global:isStaffOnDuty(targetPlayer) then
				messageToTarget = "#FF0000[PM pro tebe] #FFFFFF(" .. playerid .. ") " .. playerName .. mojeJmeno .. ": #FFAC1C" .. message
			else
				messageToTarget = "#FF0000[PM pro tebe] #FFFFFF(" .. playerid .. ") " .. playerName .. ": #FFAC1C" .. message
			end
			
			outputChatBox(messageToTarget, targetPlayer, 255, 255, 0, true)
			
			if exports.global:isStaffOnDuty(thePlayer) then
				outputChatBox("#FF0000[PM od tebe] #FFFFFF(" .. targetid .. ") " .. targetPlayerName .. targetJmeno .. ": #FFAC1C" .. message, thePlayer, 255, 255, 0, true)
			else
				outputChatBox("#FF0000[PM od tebe] #FFFFFF(" .. targetid .. ") " .. targetPlayerName ..": #FFAC1C" .. message, thePlayer, 255, 255, 0, true)
			end
			...
			if senderPmPerk and tonumber(senderPmState) == 1 and not (getElementData(targetPlayer, "reportadmin") == thePlayer) then -- if sender has pms off.
				outputChatBox("Posíláš PM zatím co blokuješ příchozí PM.", thePlayer, 200, 200, 200)
			end

			-- tells the PMing player if the target is afk.
			if getElementData(targetPlayer, "afk") then
				outputChatBox("Hráč kterému píšete PM je afk.", thePlayer, 200, 200, 200)
			end
		end
	end
end
...
function localOOC(thePlayer, commandName, ...)
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		...
		if not (...) then
			outputChatBox("Použití: /" .. commandName .. " [Message]", thePlayer, 255, 194, 14)
		elseif (muted==1) then
			outputChatBox("Jsi ztlumený.", thePlayer, 255, 0, 0)
		elseif getElementData(root, "toglooc") and not exports.global:isStaffOnDuty(thePlayer) then
			outputChatBox("LocalOOC Chat je momentálně deaktivován.", thePlayer, 255, 0, 0)
		else
...
function localDo(thePlayer, commandName, ...)
	...
	if logged==1 then
		if not (...) then
			outputChatBox("Použití: /" .. commandName .. " [Action/Event]", thePlayer, 255, 194, 14)
		else
			local message = table.concat({...}, " ")
...
function localShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		if not (...) then
			...
			outputChatBox("[" .. languagename .. "] " .. playerName .. " kričí: " .. message .. "!", thePlayer, r, g, b)
				...
				if getElementDistance( thePlayer, nearbyPlayer ) < 40 then
					...
					if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) and (nearbyPlayer~=thePlayer) then
						...
						if (logged==1) and not (isPedDead(nearbyPlayer)) then
							...
							outputChatBox("[" .. languagename .. "] " .. playerName .. " kričí: " .. message2 .. "!", nearbyPlayer, r, g, b)
						end
					end
				end
			end
...
function megaphoneShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		...
		if types[2] or types[3] or types[4] or (exports.global:hasItem(thePlayer, 141)) or ( isElement(vehicle) and exports.global:hasItem(vehicle, 141) and (seat==1 or seat==0)) then
			...
			outputChatBox("Věřte tomu nebo ne, je těžké křičet přes megafon, který nemáte.", thePlayer, 255, 0 , 0)
		end
	end
end
...
function toggleFaction(thePlayer, commandName)
	...
	if fL then
		if togState[pF] == false or not togState[pF] then
			togState[pF] = true
			outputChatBox("Chat s frakcemi je nyní deaktivován.", thePlayer)
			for index, arrayPlayer in ipairs( exports.pool:getPoolElementsByType( "player" ) ) do
				if isElement( arrayPlayer ) then
					if exports.factions:isPlayerInFaction(arrayPlayer, pF) and getElementData(thePlayer, "loggedin") == 1 then
						outputChatBox("((".. theTeamName .. ")) OOC Frakční chat zakázán", arrayPlayer, 255, 0, 0)
					end
				end
			end
		else
			...
			for index, arrayPlayer in ipairs( exports.pool:getPoolElementsByType( "player" ) ) do
				if isElement( arrayPlayer ) then
					if exports.factions:isPlayerInFaction(arrayPlayer, pF) and getElementData(thePlayer, "loggedin") == 1 then
						outputChatBox("((".. theTeamName .. ")) OOC Frakční chat povolen", arrayPlayer, 0, 255, 0)
					end
				end
			end
		end
...
function toggleFactionSelf(thePlayer, commandName)
	...
	if(logged==1) then
		...
		if (factionBlocked==1) then
			...
			outputChatBox("((".. teamName ..")) Frakční chat je nyní pro vás povolen.", thePlayer, 0, 255, 0)
		else
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "chat-system:blockF"..pF, 1, false)
			outputChatBox("((".. teamName ..")) Frakční chat je nyní pro vás povolen.", thePlayer, 255, 0, 0)
		end
	end
end
	...
	if (logged==1) then
		if not (...) then
			...
			if not playerFaction then
				outputChatBox("Nejste v této frakci.", thePlayer)
				return
			end
...
function sfpdHq(thePlayer, commandName, ...)
	...
	if factionID > 0 then
		...
		outputChatBox("Chcete-li použít tento příkaz, musíte být ve službě.", thePlayer, 255, 0, 0)	
		return
	end
	...
	if not exports.factions:hasMemberPermissionTo(thePlayer, factionID, "use_hq") then
		outputChatBox("K použití tohoto příkazu nemáte oprávnění.", thePlayer, 255, 0, 0)
	elseif #message == 0 then
		outputChatBox("Použití: /hq [message]", thePlayer, 255, 194, 14)
	else
	...
	if (logged==1) then
		if not (...) then
			...
			if not exports.factions:hasMemberPermissionTo(thePlayer, playerFaction, "use_fl") then
				outputChatBox("Nejste vůdce frakce.", thePlayer, 255, 0, 0)
			else
				local theTeam = exports.factions:getFactionFromID(playerFaction)
				local theTeamName = getTeamName(theTeam)
...
function togGovOOC(thePlayer, theCommand)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if (goocTogState == false) then
			outputChatBox("Vládní OOC byl nyní deaktivován.", thePlayer, 0, 255, 0)
			goocTogState = true
		elseif (goocTogState == true) then
			outputChatBox("Vláda OOC byla povolena.", thePlayer, 0, 255, 0)
			goocTogState = false
		else
			outputChatBox("[TG-G-C-ERR-545] Nahlaste to prosím.", thePlayer, 255, 0, 0)
		end
	end
end
...
function togGovOOCSelf(thePlayer, theCommand)
	...
	if types[2] or types[3] or types[4] and (logged==1) then
		...
		if (selfState == false) then
			outputChatBox("Government OOC byl nyní pro vás deaktivován. Dejte "..tostring(theCommand).." znovu povolit.", thePlayer, 0, 255, 0)
			setElementData(thePlayer, "chat.togGovOOCSelf", true)
		elseif (selfState == true) then
			outputChatBox("Vládní OOC bylo pro vás povoleno.", thePlayer, 0, 255, 0)
			setElementData(thePlayer, "chat.togGovOOCSelf", false)
		else
			outputChatBox("[TG-G-C-ERR-546] Nahlaste to prosím.", thePlayer, 255, 0, 0)
		end
	end
end
...
function govooc(thePlayer, commandName, ...)
	...
	if types[2] or types[3] or types[4] and (logged==1) then
		local selfState = getElementData(thePlayer, "chat.togGovOOCSelf") or false
		if selfState then
			outputChatBox("Dříve jste pro sebe vypnuli vládní chat OOC. Pomocí /toggov znovu povolte.", thePlayer, 255, 0, 0)
			return
		end
		if not (...) then
			outputChatBox("Použití: /gooc [message]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
			...
			if goocTogState == true then
				outputChatBox("Tento chat je aktuálně deaktivován.", thePlayer, 255, 0, 0)
				return
			end
...
function setRadioChannel(thePlayer, commandName, slot, channel)
	...
	if getElementData(thePlayer, "gender") == 1 then
		pohlavi = "a"
	else
		pohlavi = ""
	end
	...
	if not (channel) then
		outputChatBox("Použití: /" .. commandName .. " [Slot] [Frekvence]", thePlayer, 255, 194, 14)
	else
						...
						if tonumber(v[2]) > 0 then
							if channel > 1 and channel < 1000000000 and hasChannelAccess(thePlayer, channel) then
								if exports['item-system']:updateItemValue(thePlayer, k, channel) then
									outputChatBox("Přeladili jste si vysílačku na kanál #" .. channel .. ".", thePlayer)
									triggerEvent('sendAme', thePlayer, "si přeladil"..pohlavi.." vysílačku.")
								end
							else
								outputChatBox("Na tu frekvenci nemůžete naladit vysílačku!", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Rádio je vypnuté. ((/toggleradio))", thePlayer, 255, 0, 0)
						end
						return
					end
				end
			end
			outputChatBox("Nemáte tolik rádií.", thePlayer, 255, 0, 0)
		else
			outputChatBox("Nemáte rádio!", thePlayer, 255, 0, 0)
		end
	end
end
...
function toggleRadio(thePlayer, commandName, slot)
	if (exports.global:hasItem(thePlayer, 6)) then
		...
		if titemValue < 0 then
			outputChatBox("Zapnul jsi rádio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "turns  radio on.")
		else
			outputChatBox("Vypnul jsi rádio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "turns  radio off.")
		end
...
function adminChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer))  then
		if not (...) then
			outputChatBox("Použití: /a [Message]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
	...
	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer)  or exports.integration:isPlayerSupporter(thePlayer))  then
		if not (...) then
			...
			if getElementData(thePlayer, "hideg") then
				...
				outputChatBox("Gamemaster Chat - UKÁZÁNÍ",thePlayer)
			end
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
				...
				if logged==1 and (exports.integration:isPlayerTrialAdmin(arrayPlayer) or exports.integration:isPlayerSupporter(arrayPlayer)) then
					...
					if hideg then
						...
						if string.find(string, account) then
							...
							outputChatBox("Uvedeno v /g chat - "..accountName..": "..message, arrayPlayer)
						end
					else
						table.insert(affectedElements, arrayPlayer)
...
function toggleAdminChat(thePlayer, commandName)
	...
	if logged==1 and exports.integration:isPlayerLeadAdmin(thePlayer) then
		...
		if not hidea or hidea == "false" then
			...
			outputChatBox("Na vašem suti se přestal zobrazovat administrátorský chat, /toga znovu povolit.",thePlayer, 0,255,0)
		elseif hidea=="true" then
			setElementData(thePlayer, "hidea", "false")
			outputChatBox("Na vaší obrazovce se začal zobrazovat administrátorský chat, /toga znovu povolit.",thePlayer, 0,255,0)
		end
	end
end
...
function toggleGMChat(thePlayer, commandName)
	...
	if logged==1 and (exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		...
		outputChatBox("Gamemaster Chat - "..(hideg and "ZOBRAZIT" or "SCHOVAT").." /togg pro zobrazení.",thePlayer)
	end
end
...
function toggleOOC(thePlayer, commandName)
	...
	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local players = exports.pool:getPoolElementsByType("player")
		local oocEnabled = exports.global:getOOCState()
		local loocEnabled = getElementData(root, "toglooc")
		if (commandName == "togooc") then
			if (oocEnabled==0) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) then
						outputChatBox("OOC Chat povolen správcem.", arrayPlayer, 0, 255, 0)
					end
				end
			elseif (oocEnabled==1) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) then
						outputChatBox("OOC Chat deaktivován správcem.", arrayPlayer, 255, 0, 0)
					end
				end
			end
			...
			if (oocEnabled==0) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) and (tonumber(admin)>0)then
						outputChatBox("OOC Chat povolen tiše administrátorem. " .. getPlayerName(thePlayer) .. ".", arrayPlayer, 0, 255, 0)
					end
				end
			elseif (oocEnabled==1) then
				exports.global:setOOCState(0)
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) and (tonumber(admin)>0)then
						outputChatBox("OOC Chat deaktivován tiše administrátorem. " .. getPlayerName(thePlayer) .. ".", arrayPlayer, 255, 0, 0)
					end
				end
			end
		elseif (commandName == "toglooc") then
			if loocEnabled then
				setElementData(root, "toglooc", false)

				for k, arrayPlayer in ipairs(players) do
					local logged = getElementData(arrayPlayer, "loggedin")

					if (logged==1) then
						outputChatBox("LocalOOC Chat povolen správcem.", arrayPlayer, 0, 255, 0)
					end
				end
			elseif not loocEnabled then
				setElementData(root, "toglooc", true)

				for k, arrayPlayer in ipairs(players) do
					local logged = getElementData(arrayPlayer, "loggedin")

					if (logged==1) then
						outputChatBox("LocalOOC Chat deaktivován správcem.", arrayPlayer, 255, 0, 0)
					end
				end
			end
		end
...
addCommandHandler("toglooc", toggleOOC, false, false)
...
function togglePM(thePlayer, commandName)
	...
	if hasPerk or (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerFMTLeader(thePlayer)) then
		if tonumber(value)== 1 then
			outputChatBox("PM jsou nyní aktivované.", thePlayer, 0, 255, 0)
			exports.donators:updatePerkValue(thePlayer, 1, 0)
		else
			outputChatBox("PM jsou nyní deaktivovány.", thePlayer, 255, 0, 0)
			exports.donators:updatePerkValue(thePlayer, 1, 1)
		end
	else
		outputChatBox("Tuto výhodu nemáte aktivovanou. Navštivte donátor výhody v nabídce F10.", thePlayer)
	end
end
...
function payPlayer(thePlayer, commandName, targetPlayerNick, amount)
	...
	if (logged==1) then
		if not (targetPlayerNick) or not (amount) or not tonumber(amount) then
			outputChatBox("Použití: /" .. commandName .. " [Player Partial Nick] [Amount]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
			...
			if targetPlayer then
				...
				if (distance<=10 and not getElementData(targetPlayer, "reconx")) then
					...
					if (targetPlayer==thePlayer) then
						outputChatBox("Nemůžete platit peníze sobě.", thePlayer, 255, 0, 0)
					elseif amount == 0 then
						outputChatBox("Musíte zadat částku větší než 0.", thePlayer, 255, 0, 0)
					elseif (hoursplayed<5) and (amount>50) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) then
						outputChatBox("Musíte hrát alespoň 5 hodin aby jste mohl provést tuto větší transakci...", thePlayer, 255, 0, 0)
					elseif exports.global:hasMoney(thePlayer, amount) then
						if hoursplayed < 5 and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) then
							...
							if totalAmount > 200 then
								outputChatBox( "Můžete zaplatit pouze určitou částku za pět minut. /report pro administrátora k převodu většího množství hotovosti.", thePlayer, 255, 0, 0 )
								return
							end
						...
						if (hoursplayed<5) then
							exports.global:sendMessageToAdmins("AdmWarn: Nový hráč '" .. getPlayerName(thePlayer) .. "' dal " .. exports.global:formatMoney(amount) .. "$ hráči '" .. targetPlayerName .. "'.")
						end
						...
						triggerEvent('sendAme', thePlayer, "bere peníze z peněženky a dává je " .. targetPlayerName .. ".")
						outputChatBox("Dal jsi " .. exports.global:formatMoney(amount) .. "$ hráči " .. targetPlayerName .. ".", thePlayer)
						outputChatBox(getPlayerName(thePlayer) .. " ti dal " .. exports.global:formatMoney(amount) .. "$.", targetPlayer)

						exports.global:applyAnimation(thePlayer, "DEALER", "shop_pay", 4000, false, true, true)
					else
						outputChatBox("Nemáš dost peněz.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Jste příliš daleko od " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function localWhisper(thePlayer, commandName, targetPlayerNick, ...)
	...
	if (logged==1) then
		if not (targetPlayerNick) or not (...) then
			outputChatBox("Použití: /" .. commandName .. " [Player Partial Nick / ID] [Message]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
			...
			if targetPlayer then
				...
				if (targetPlayerDimension==dimension) and (targetPlayerInterior==interior) then
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<3) then
						...
						triggerEvent('sendAme', thePlayer, "šeptá do " .. targetPlayerName .. ".")
						local r, g, b = 255, 255, 255
						local focus = getElementData(thePlayer, "focus")
						if type(focus) == "table" then
							for player, color in pairs(focus) do
						...
						outputChatBox("[" .. languagename .. "] " .. name .. " šeptá: " .. message, thePlayer, r, g, b)
						local r, g, b = 255, 255, 255
						...
						outputChatBox("[" .. languagename .. "] " .. name .. " šeptá: " .. message2, targetPlayer, r, g, b)
											...
											if playerVeh then
												if pVeh then
													if pVeh==playerVeh then
														outputChatBox("[" .. languagename .. "] " .. name .. " šeptá do " .. getPlayerName(targetPlayer):gsub("_"," ") .. ": " .. message2, p, 255, 255, 255)
													end
												end
											else
												outputChatBox("[" .. languagename .. "] " .. name .. " šeptá do " .. getPlayerName(targetPlayer):gsub("_"," ") .. ": " .. message2, p, 255, 255, 255)
							...
							if not exports.vehicle:isVehicleWindowUp(playerCar) then
								if pveh then
									if playerCar == pveh then
										...
										outputChatBox( "[" .. languagename .. "] " .. name .. " šeptá: " .. message2, targetPlayers, r, g, b)
	...
	if (logged==1) then
		...
		if exports.factions:isInFactionType(thePlayer, 6)then -- news faction
			if not (targetPartialPlayer) then
				outputChatBox("Použití: /" .. commandName .. " [Player Partial Nick]", thePlayer, 255, 194, 14)
			else
				...
				if targetPlayer then
					...
					if (targetLogged==1) then
						if(getElementData(targetPlayer,"interview"))then
							outputChatBox("S tímto hráčem již probíhá rozhovor..", thePlayer, 255, 0, 0)
						else
							exports.anticheat:changeProtectedElementDataEx(targetPlayer, "interview", true, false)
							local playerName = getPlayerName(thePlayer)
							outputChatBox(playerName .." vám nabídl rozhovor.", targetPlayer, 0, 255, 0)
							outputChatBox("((Během rozhovoru použijte /i k rozhovoru.))", targetPlayer, 0, 255, 0)
							local NewsFaction = exports.factions:getPlayersInFaction(20)
							for key, value in ipairs(NewsFaction) do
								outputChatBox("((".. playerName .." pozval " .. targetPlayerName .. " na pohovor.))", value, 0, 255, 0)
							end
						end
					end
				end
				...
				if targetPlayer then
					...
					if (targetLogged==1) then
						if not(getElementData(targetPlayer,"interview"))then
							outputChatBox("S tímto hráčem není veden rozhovor.", thePlayer, 255, 0, 0)
						else
							exports.anticheat:changeProtectedElementDataEx(targetPlayer, "interview", false, false)
							local playerName = getPlayerName(thePlayer)
							outputChatBox(playerName .." ukončil váš rozhovor.", targetPlayer, 255, 0, 0)

							local NewsFaction = exports.factions:getPlayersInFaction(20)
							for key, value in ipairs(NewsFaction) do
								outputChatBox("((".. playerName .." skončil " .. targetPlayerName .. "rozhovor.))", value, 255, 0, 0)
							end
						end
					end
				end
	...
	if (logged==1) then
		if(getElementData(thePlayer, "interview"))then
			if not(...)then
				...
				local finalmessage = "[ZPRÁVY] Interview host " .. name .." říká: ".. message
				if exports.factions:isInFactionType(thePlayer, 6)then -- news faction
					finalmessage = "[ZPRÁVY] " .. name .." říká: ".. message
				end
				...
				exports.logs:dbLog(thePlayer, 23, thePlayer, "ZPRÁVY " .. message)
...
function charityCash(thePlayer, commandName, amount)
	if not (amount) then
		outputChatBox("Použití: /" .. commandName .. " [Částka]", thePlayer, 255, 194, 14)
	else
		local donation = tonumber(amount)
		if (donation<=0) or string.find(donation, "%.") then
			outputChatBox("Částka musí být věčší než 0.", thePlayer, 255, 0, 0)
		else
			if not exports.global:takeMoney(thePlayer, donation) then
				outputChatBox("Nemáš tolik peněz.", thePlayer, 255, 0, 0)
			elseif getElementData(thePlayer, "charita") then
				outputChatBox("Nedávej na charitu peníze tak často.", thePlayer, 255, 0, 0)
			else
				setElementData(thePlayer, "charita", true)
				outputChatBox("Dal jsi na charitu ".. exports.global:formatMoney(donation) .."$.", thePlayer, 0, 255, 0)
				exports.global:sendMessageToAdmins("AdmWrn: " ..getPlayerName(thePlayer).. " dal na charitu " ..exports.global:formatMoney(donation).."$")
				exports.logs:dbLog(thePlayer, 25, thePlayer, "CHARITY $" .. amount)
				
				setTimer(function()
					setElementData(thePlayer, "charita", false)
				end, 10000, 1)
				
				if donation > 24999 then
				    outputChatBox(""..getPlayerName(thePlayer).." daroval "..amount.." na charitu.", root, 25, 255, 25)
				end
			end
		end
	end
end
addCommandHandler("charita", charityCash, false, false)
...
function bigEars(thePlayer, commandName, targetPlayerNick)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		if not current and not targetPlayerNick then
			outputChatBox("Použití: /" .. commandName .. " [player]", thePlayer, 255, 194, 14)
		elseif current and not targetPlayerNick then
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "bigears", false, false)
			outputChatBox("Velké uši jsou vypnuté.", thePlayer, 255, 0, 0)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)

			if targetPlayer then
				outputChatBox("Nyní poslouchám " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
...
function removeBigEars()
	for key, value in pairs( getElementsByType( "player" ) ) do
		if isElement( value ) and getElementData( value, "bigears" ) == source then
			...
			outputChatBox("Velké uši jsou vypnuté (Hráč odešel).", value, 255, 0, 0)
		end
	end
end
...
function bigEarsFaction(thePlayer, commandName, factionID)
	if exports.integration:isPlayerAdmin(thePlayer) then
		...
		if not current and not factionID then
			outputChatBox("Použití: /" .. commandName .. " [faction id]", thePlayer, 255, 194, 14)
		elseif current and not factionID then
			...
			if not team then
				outputChatBox("Nebyla nalezena žádná frakce s tímto ID.", thePlayer, 255, 0, 0)
			else
				outputChatBox("Nyní poslouchám " .. getTeamName(team) .. " OOC Chat.", thePlayer, 0, 255, 0)
	...
	if targetPlayer then
		...
		if targetPlayer then
			...
			if focus[targetPlayer] and not r then
				outputChatBox( "Přestal jsi zvýrazňovat " .. string.format("#%02x%02x%02x", unpack( focus[targetPlayer] ) ) .. targetPlayerName .. "#ffc20e.", thePlayer, 255, 194, 14, true )
				...
				outputChatBox( "Nyní zvýrazňujete " .. string.format("#%02x%02x%02x", unpack( focus[targetPlayer] ) ) .. targetPlayerName .. "#00ff00.", thePlayer, 0, 255, 0, true )
			end
		...
		outputChatBox( "Chcete -li někoho přidat, /" .. commandName .. " [player] [optional red/green/blue], jen odstranit /" .. commandName .. " [player] znovu.", thePlayer, 255, 194, 14)
	end
end
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if logged==1 and isPlayerStaff(arrayPlayer) then
					...
					if hideStaffChat then
						...
						table.insert(affectedElements, arrayPlayer)
						outputChatBox("[STAFF CHAT] "..exports.global:getPlayerFullIdentity(thePlayer)..": "..message, arrayPlayer, 153, 51, 255)
					end
				end
			end
...
function businessOOC(thePlayer, commandName, business, ...)
	...
	if (logged==1) then
		if not business then
			outputChatBox("Použití: /" .. commandName .. " [Message]", thePlayer, 255, 194, 14)
			outputChatBox("OR Použití: /" .. commandName .. " [Business] [Message]", thePlayer, 255, 194, 14)
		else
			local playerName = getPlayerName( thePlayer ):gsub( "_", " ")
			local message = table.concat({...}, " ")
...
function toggleBusinessSelf(thePlayer, commandName)
	...
	if(logged==1) then
		...
		if (BusinessBlocked==1) then
			...
			outputChatBox("Obchodní chat je nyní pro vás povolen.", thePlayer, 0, 255, 0)
		else
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "chat-system:blockB", 1, false)
			outputChatBox("Obchodní chat je nyní pro vás deaktivován.", thePlayer, 255, 0, 0)
		end
	end
end
...
local mir = {
	...
	"Pokud nemáte právníka, bude vám jeden zdarma přidělen.",
}
...
addCommandHandler("prava", pdmir, false, false)

function helpchat(thePlayer, commandName, ...)
	local message = table.concat({...}, " ")
	local logged = getElementData(thePlayer, "loggedin")
	if getElementData(thePlayer, "spamDO") then
		return
	end
    if not getElementData(thePlayer, "adminjailed") and (logged==1) then
		if not getElementData(root, "toghc") then
			for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
				if getElementData(value, "loggedin") == 1 then
					if not getElementData(value, "hcBlock") then
						outputChatBox("(( [HELPCHAT] "..(exports.global:getPlayerFullIdentity(thePlayer))..": "..message.." ))", value, 24, 200, 54)
					end
				end
			end
			...
			outputChatBox("Help Chat je momentálně deaktivován.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("hc", helpchat)

function togHC(thePlayer, commandName)
	local logged = getElementData(thePlayer, "loggedin")

	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local players = exports.pool:getPoolElementsByType("player")
		local togHCCheck = getElementData(root, "toghc")
		if not togHCCheck then
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				setElementData(root, "toghc", true)
				if	(logged==1) then
					outputChatBox("Help Chat deaktivován správcem.", arrayPlayer, 255, 0, 0)
				end
			end
		else
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				setElementData(root, "toghc", false)
				if	(logged==1) then
					outputChatBox("Help Chat aktivován správcem.", arrayPlayer, 0, 255, 0)
				end
			end
		end
	end
end
addCommandHandler("atoghc", togHC, false, false)

function hcBlock(thePlayer, commandName)
	local logged = getElementData(thePlayer, "loggedin")

	if(logged==1)then
		if getElementData(thePlayer, "hcBlock") then
			setElementData(thePlayer, "hcBlock", false)
			outputChatBox("Zapnul jste si zobrazování Helpchatu.", thePlayer, 0, 255, 0)
		else
			setElementData(thePlayer, "hcBlock", true)
			outputChatBox("Vypnul jste si zobrazování Helpchatu.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("toghc", hcBlock, false, false)
