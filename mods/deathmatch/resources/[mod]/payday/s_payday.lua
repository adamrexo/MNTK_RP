		...
		for int_id, cost in pairs( rentalHouses[ dbid ] ) do
			if bankmoney >= cost then
				...
				if exports.global:isResourceRunning("interior_system") then
					...
					if account then
						exports.announcement:makePlayerNotification(account.id, characterName.." nemožno si vybavit nemovitost, nedostatek financí #"..int_id..".", "You have lost this interior because the rental contract was broken.\nCould not afford $"..exports.global:formatMoney(cost).." for the rent of the interior.", "interior_inactivity_scanner")
					end
					-- leave some history for administrative purposes.
					exports["interior-manager"]:addInteriorLogs(int_id, "Nedostatek financí $"..exports.global:formatMoney(cost)..")", player)
				end
			end
		end
	end
