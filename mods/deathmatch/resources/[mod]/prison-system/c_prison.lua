...
function PrisonGUIF(result)
        ...
        PrisonGUI.window[1] = guiCreateWindow(x, y, w, h, "Vězeňský systém", false)
        ...
        PrisonGUI.column[1] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Unikátní ID", 0.1)
        PrisonGUI.column[7] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Cela", 0.05)
        PrisonGUI.column[2] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Jmeno", 0.2)
        PrisonGUI.column[3] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Datum odsouzení", 0.15)
        PrisonGUI.column[4] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Propuštění dny", 0.1)
        PrisonGUI.column[8] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Propuštění hodiny", 0.1)
        PrisonGUI.column[10] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Online minuty", 0.08)
        PrisonGUI.column[5] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Naposledy aktualizováno", 0.1)
        PrisonGUI.column[9] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Pokuta", 0.1)
        PrisonGUI.column[6] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Poplatky", 0.5)

        PrisonGUI.button[1] = guiCreateButton(13, 241, 100, 73, "Zavřít", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[1], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[2] = guiCreateButton(129, 243, 119, 30, "Propustit vězně", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[2], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[3] = guiCreateButton(130, 280, 119, 30, "Přidat nového vězně", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[3], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[4] = guiCreateButton(263, 243, 119, 30, "Aktualizujte vězně", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[4], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[5] = guiCreateButton(588, 242, 134, 30, "Přepnout číslo brány", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[5], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[6] = guiCreateButton(588, 282, 134, 30, "Přepnout všechny brány", false, PrisonGUI.window[1])
        ...
        if pd_offline_jail or exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
            PrisonGUI.button[7] = guiCreateButton(263, 280, 119, 30, "Přidat offline vězně", false, PrisonGUI.window[1])
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[2], function()
            ...
            if r>=0 and c>=0 then
                ...
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Musíte být poblíž tohoto vězně, abyste je odstranili.", 255, 0, 0)
                    return
                end
                triggerServerEvent("removePrisoner", resourceRoot, removeid, true)
            else
                outputChatBox("Nejprve proveďte výběr.", 255, 0, 0)
            end
            end, false)
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[4], function()
            ...
            if r>=0 and c>=0 then
                local targetPlayer = getPlayerFromName(guiGridListGetItemText(PrisonGUI.gridlist[1], r, 3))
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Musíte být poblíž tohoto vězně, abyste je aktualizovali.", 255, 0, 0)
                    return
                end
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[6], function() -- Toggle all gates
                if getElementDimension(getLocalPlayer(  )) == gateDim then
                    ...
                    gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Přepnout všechny brány", false)
                    guiWindowSetSizable(gui["_root"], false)

                    gui["label"] = guiCreateLabel(10, 35, 231, 56, "Opravdu chcete přepnout\n všechny brány ve vězení?", false, gui["_root"])
                    guiLabelSetHorizontalAlign(gui["label"], "center", false)
                    guiLabelSetVerticalAlign(gui["label"], "center")

                    gui["pushButton"] = guiCreateButton(30, 95, 81, 31, "Ano", false, gui["_root"])
                    ...
                    gui["pushButton_2"] = guiCreateButton(150, 95, 81, 31, "Ne", false, gui["_root"])
                        addEventHandler("onClientGUIClick", gui["pushButton_2"], function()
                            destroyElement(gui["_root"])
                            local gui = nil
                        end, false)
                else
                    outputChatBox("Musíte být ve stejném interiéru/dimenzi jako brány.", 255, 0, 0)
                end
            end, false)
...
function addPrisonerGUI(source, name, cell, days, hours, charges, row, fines, jailID)
    ...
    if arrestLocation then
        ...
        newPrisonerGUI.window[1] = guiCreateWindow(x, y, w, h, "Přidat nového vězně", false)
        guiWindowSetSizable(newPrisonerGUI.window[1], false)

        newPrisonerGUI.button[1] = guiCreateButton(13, 241, 123, 28, "Zavřít", false, newPrisonerGUI.window[1])
        guiSetProperty(newPrisonerGUI.button[1], "NormalTextColour", "FFAAAAAA")
        newPrisonerGUI.button[2] = guiCreateButton(170, 241, 123, 28, "Přidat vězně", false, newPrisonerGUI.window[1])
        ...
        if source==PrisonGUI.button[3] then
            newPrisonerGUI.label[1] = guiCreateLabel(13, 22, 67, 15, "Jmeno/ID:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")
            newPrisonerGUI.label[4] = guiCreateLabel(226, 25, 72, 17, "Nenalezeno", false, newPrisonerGUI.window[1])
        ...
        elseif source==PrisonGUI.button[7] then
            newPrisonerGUI.label[1] = guiCreateLabel(15, 22, 67, 15, "Přesný název:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")

            guiSetEnabled(newPrisonerGUI.edit[5], false)
        end

        newPrisonerGUI.label[2] = guiCreateLabel(12, 147, 77, 15, "Poplatky:", false, newPrisonerGUI.window[1])
        ...
        newPrisonerGUI.label[8] = guiCreateLabel(120, 48, 30, 15, "Pokuta:", false, newPrisonerGUI.window[1])
        guiSetFont(newPrisonerGUI.label[8], "default-bold-small")
        guiEditSetMaxLength(newPrisonerGUI.edit[5], 6)

        newPrisonerGUI.label[5] = guiCreateLabel(13, 96, 51, 15, "Dni:", false, newPrisonerGUI.window[1])
        guiSetFont(newPrisonerGUI.label[5], "default-bold-small")
        newPrisonerGUI.label[6] = guiCreateLabel(7, 77, 301, 15, "============Čas odsouzení==========", false, newPrisonerGUI.window[1])
        guiLabelSetHorizontalAlign(newPrisonerGUI.label[6], "center", false)
        newPrisonerGUI.label[7] = guiCreateLabel(148, 96, 51, 15, "Hodiny:", false, newPrisonerGUI.window[1])
        ...
        if name then
            ...
            guiSetText(newPrisonerGUI.window[1], "Aktualizujte Vězne") -- All about the cosmedics baby
            guiSetText(newPrisonerGUI.button[2], "Aktualizujte Vězne")
        ...
        addEventHandler("onClientGUIClick", newPrisonerGUI.button[2], function()
            if overLimit(guiGetText(newPrisonerGUI.edit[3]), guiGetText(newPrisonerGUI.edit[4])) then -- Returns true if over hour limit
                outputChatBox("Pokoušíte se někoho uvěznit "..tonumber(hourLimit).." hodin přes limit.", 255, 0, 0) return end

            local item = guiComboBoxGetSelected(newPrisonerGUI.edit[2])
            if item==-1 then
                outputChatBox("Nejprve proveďte výběr cely.", 255, 0, 0) return end
            ...
            if string.len(guiGetText(newPrisonerGUI.edit[1])) > 0 then
                ...
                if source==PrisonGUI.button[3] then -- Online add
                    if tonumber(guiGetText(newPrisonerGUI.edit[5])) <= 100000 then
                        if tonumber(guiGetText(newPrisonerGUI.edit[4]))+tonumber(guiGetText(newPrisonerGUI.edit[3])) >= 1 then
                            ...
                            outputChatBox("Zadejte prosím vyšší částku odsouzení.", 255, 0, 0)
                        end
                    else
                        outputChatBox("Maximální výše pokuty je $100,000.", 255, 0, 0)
                    end
                ...
                outputChatBox("Zadali jste jméno?", 255, 0, 0)
            end
        end, false)
        ...
        outputChatBox("Musíte být v oblasti pro zatčeni.", 255, 0, 0)
    end
end
...
function toggleGate()
    if isElement(ToggleGateGUI.window[1]) then
        ...
        local w, h = 272, 200
        local x, y = (scr[1]/2)-(w/2), (scr[2]/2)-(h/2)
        ToggleGateGUI.window[1] = guiCreateWindow(x, y, w, h, "Přepnout bránu", false)
        ...
        ToggleGateGUI.button[1] = guiCreateButton(11, 54, 120, 37, "Přepnout bránu", false, ToggleGateGUI.window[1])
        guiSetProperty(ToggleGateGUI.button[1], "NormalTextColour", "FFAAAAAA")
        ToggleGateGUI.button[2] = guiCreateButton(136, 54, 120, 37, "Zrušit", false, ToggleGateGUI.window[1])
        ...
        ToggleGateGUI.label[1] = guiCreateLabel(183, 120, 45, 20, "Otevřeno", false, ToggleGateGUI.window[1])
        ToggleGateGUI.label[2] = guiCreateLabel(183, 140, 40, 20, "Zrušit", false, ToggleGateGUI.window[1])
...
function prisonPed()
    local ped = createPed(283, 1585.9970703125, -1698.5009765625, 13.589937210083)
    setPedAnimation(ped, "COP_AMBIENT", "Coplook_loop", -1, true, false, false)
    setElementFrozen(ped, true)
    setElementRotation(ped, 0, 0, 58)
