...
function showRespawnButton(victimDropItem)
	...
	respawnFont = guiCreateFont( "RespawnFont.ttf", 12 )
	bRespawn = guiCreateButton(x, y, width, height,"Respawn",false)
		guiSetFont(bRespawn, respawnFont)
	...
	timer1 = setTimer(function()
		...
		guiSetText(bRespawn, "Respawn\nDostupný za "..convertTime(remaining))
...
function convertTime(ms)
    if ms and type(ms) == "number" then
        ...
        return string.format("%02d:%02d", (mins - hours * 60), secs)
    end
end
