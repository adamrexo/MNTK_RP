...
function playerDeath(totalAmmo, killer, killerWeapon)
	if getElementData(source, "dbid") then
		...
		elseif getElementData(source, "jailed") then
			...
			outputChatBox("Pokud vaše smrt byla zapříčiněna hráčem který porušoval pravidla udělejte si report a admin vás oživí.", source)
			outputChatBox("Pokud přijmete svou smrt, můžete ztratit některé ze svých předmětů.", source)
...
function acceptDeath(victimDropItem)
	if isPedDead(client) then
		...
		outputChatBox("Respawnuju...", client)
...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		outputChatBox("Dostali jste ošetření od Emergency.", thePlayer, 255, 255, 0)
		...
		if count > 0 then
			outputChatBox("Zaměstnanec Emergency: Předali jsme vaše drogy LSPD..", thePlayer, 255, 194, 14)
		end
		...
		if (removedWeapons~=nil) then
			if gunlicense == 0 and factiontype ~= 2 then
				outputChatBox("Zaměstnanec Emergency: Odebrali jsme zbraně, na které jste neměli povolení. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			else
				outputChatBox("Zaměstnanec Emergency: Odebrali jsme zbraně, které nesmíte nosit. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			end
		end
