...
function frames_showFrameGUI ( itemSlot )
	if not frameGUI["window"] then
		...
		frameGUI["window"] = guiCreateWindow ( x, y, width, height, "Aktualizovat adresu URL obrázku", false )
		frameGUI["url"] = guiCreateEdit ( 10, 25, 379, 25, "http://www.example.com/picture.png", false, frameGUI["window"] )
		frameGUI["save"] = guiCreateButton ( 10, 55, 379, 25, "Uložit", false, frameGUI["window"] )
		frameGUI["cancel"] = guiCreateButton ( 10, 85, 379, 25, "Zrušit", false, frameGUI["window"] )
...
function isURLValid ( url )
	...
	return false, "Neplatné rozšíření obrázku. Přijímané typy jsou: " .. _extensions
end
			...
			if rendering then
				...
				if texture then
					...
					if width > 1000 or height > 1000 then
						outputChatBox ( "Textury nemohou mít šířku a výšku větší než 1000 pixelů", 255, 0, 0, false )
						return
					end
				end
...
function frames_renderTextureSelection ( )
	...
	dxDrawText ( "Další: " .. KEYS.NEXT, sw * 0.02, sh * 0.63, sw, sh, tocolor ( 255, 255, 255, 255 ), 1.0, "default-bold" )
	dxDrawText ( "Předchozí: " .. KEYS.PREVIOUS, sw * 0.02, sh * 0.65, sw, sh, tocolor ( 255, 255, 255, 255 ), 1.0, "default-bold" )
	dxDrawText ( "Potvrdit: " .. KEYS.CONFIRM, sw * 0.02, sh * 0.67, sw, sh, tocolor ( 255, 255, 255, 255 ), 1.0, "default-bold" )
	dxDrawText ( "Zrušit: " .. KEYS.CANCEL, sw * 0.02, sh * 0.69, sw, sh, tocolor ( 255, 255, 255, 255 ), 1.0, "default-bold" )
