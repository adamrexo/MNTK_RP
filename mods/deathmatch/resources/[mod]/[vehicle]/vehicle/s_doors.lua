...
function controlRamp(theVehicle)
	...
	if not (isElement(theVehicle) and theVehicle == playerVehicle) then
		outputChatBox("Potřebuješ být ve vozidle aby jsi použil tohle tlačítko.", client, 255, 0, 0)
		return
	end
	
	if not (exports['item-system']:hasItem(theVehicle, 117)) then
		outputChatBox("Potřebuješ mit danou věc v inventáři pokud chceš tohle udělat!", client, 255, 0, 0)
		return
	end

	if not (getElementData(theVehicle, "handbrake") == 1) then
		outputChatBox("Potřebuješ mit nastavenou ruční brzdu pokud chceš sundat rampu!", client, 255, 0, 0)
		return
	end
	
	if not (getElementModel(theVehicle) == 578) then
		outputChatBox("Tohle vozidlo neni kompatibilní s rampou!", client, 255, 0, 0)
		return
	end
