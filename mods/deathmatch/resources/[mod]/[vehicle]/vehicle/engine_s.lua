...
function startEngine( players, veh )
	if not startings[ veh ] or getTickCount() - startings[ veh ] > 5000 then
		...
		if ( ( getElementData( veh, 'battery' ) or 100 ) > 0 ) or batteryExceptions( veh ) then
			...
			triggerEvent('sendAme', client, "snažil se nastartovat vozidlo ale neuspěl.")
			status = 'nobatt'
			return
		end
		...
		if status == 'ok' then
			...
			-- logs
			exports.vehicle_manager:addVehicleLogs( vid , "nastartoval vozidlo", client )
			exports.logs:dbLog( client, 31, { veh, client } , "nastartoval vozidlo" )
		elseif status == 'hotwired' then 
			...
			triggerEvent('sendAme', client, "natahl se dolu po pár káblu.")

			-- logs
			exports.vehicle_manager:addVehicleLogs( vid , "Nastartoval vozidlo pomoci káblu.", client )
			exports.logs:dbLog( client, 31, { veh, client } , "Nastartoval vozidlo které už bylo nastartovane pomoci drátu" )
		else
			...
			setTimer(function (client)
				...
				elseif status == 'enginebroke' then
					exports.hud:sendBottomNotification( client, exports.global:getVehicleName( veh ), "Motor je rozbitý." )
					triggerEvent('sendAme', client, "snažil se nastartovat vozidlo ale neuspěl.")
				elseif status == 'nofuel' then
					exports.hud:sendBottomNotification( client, exports.global:getVehicleName( veh ), "Neni dostatek PHM" )
					triggerEvent('sendAme', client, "snažil se nastartovat vozidlo ale neuspěl.")
				end
			end, 1500, 1, client)
		end
