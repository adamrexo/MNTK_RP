...
function build_SaleGUI(itemValue)
	...
	gui["label_2"] = guiCreateLabel(30, 75, 331, 21, "Při podpisu dávate plne prava na prodej jiné osobě", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["label_2"], "left", false)
	guiLabelSetVerticalAlign(gui["label_2"], "center")

	gui["label_3"] = guiCreateLabel(30, 95, 331, 21, "prava pro vozidlo byla převedena dané osobě.", false, gui["_root"])
	...
	gui["label_4"] = guiCreateLabel(30, 135, 150, 16, "Nové jmeno majitele", false, gui["_root"])
...
function openVehSales(cmd, ...)
	...
	if veh then
		...
		if (exports.integration:isPlayerAdmin(localPlayer) and getElementData(localPlayer, "duty_admin") == 1 ) or (getElementData(veh, "owner") == getElementData(localPlayer, "dbid")) or (leader and faction) then
			if not (...) then
				return outputChatBox("SYNTAX: /" .. cmd .. " [kousek jména / id]", 255, 194, 14)
			end

			exports['item-system']:playSoundInvOpen()

			GUIEditor.window[1] = guiCreateWindow(1107, 377, 391, 181, "Prodat vozidlo", false)
			guiWindowSetSizable(GUIEditor.window[1], false)
			exports.global:centerWindow(GUIEditor.window[1])

			GUIEditor.label[1] = guiCreateLabel(35, 36, 323, 79, "Prodávaš vozidlo s id # () to hráčovi.\n\n", false, GUIEditor.window[1])
			...
			return outputChatBox("Nevlastníš tohle vozidlo", 255, 0, 0)
		end
	end
end
...
addEventHandler("onClientVehicleExit", getRootElement(), function(thePlayer)
	if thePlayer == localPlayer and (getVehicleType(source) == "Bike" or getVehicleType(source) == "BMX") and not clientStartExit then
		...
		if isVehicleLocked(source) then
			...
			outputChatBox("Tvoje kolo bylo automaticky uzamknuto protože jsi upadl.", 255, 0, 0)
		end
		clientStartExit = false
