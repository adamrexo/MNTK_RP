...
local function printVehicleCreateSyntax( player, command )
	outputChatBox("SYNTAX: /" .. command .. " [ID from Veh Lib] [barva1] [barva2] [Majitel] [Frakční vozidlo (1/0)] [-1=carshop cena] [Tint] ", player, 255, 194, 14)
...
function createPermanentVehicle(player, command, ...)
	if hasCreatePermission( player ) then
		...
		if not vehShopData then
			outputChatBox('Nesprávny ID vozidla, prosím vyber správne ID z /vehlib.', player, 255, 100, 100)
			return
		end
		...
		if not id then -- the vehicle doesn't have a proper ID
			outputChatBox('Vozidlo nema nastaven MTA model.', player, 255, 100, 100)
			return
		end
		...
		if isFactionOwned then
			...
			if not theTeam then
				outputChatBox("Neni možné najit frakci s tím ID: " .. owner .. ".", player, 255, 100, 100 )
				return
			end
			factionID = tonumber(owner)
			owner = -1
			if not exports.global:takeMoney(theTeam, cost) then
				outputChatBox("[MAKEVEH] Tahle frakce si nemůže dovolit vozidlo.", player, 255, 100, 100)
				outputChatBox("Ta frakce si nemůže dovolit vozidlo..", player, 255, 100, 100)
				return
			end
		else
			...
			if other then
				...
				if not exports.global:canPlayerBuyVehicle(other) then
					outputChatBox("[MAKEVEH] Tenhle hráč ma max. počet vozidel.", player, 255, 0, 0)
					outputChatBox("Máč max. počet vozidel.", other, 255, 0, 0)
					return
				elseif not exports.global:takeMoney(other, cost) then
					outputChatBox("[MAKEVEH] Tenhle hráč si nemůže dovolit vozidlo.", player, 255, 0, 0)
					outputChatBox("Nemůžeš si dovolit vozidlo.", other, 255, 0, 0)
					return
				end
			else
		...
		if not veh then
			outputChatBox("Nespravně specifikovaný MTA model.", player, 255, 100, 100)
			return
		end
		...
		.. "dimension=?, currdimension=?, tintedwindows=?, variant1=?, variant2=?, creationDate=NOW(), createdBy=-1, `vehicle_shop_id`=? ", -- změněno createdBy z ? na -1 kvůli opravy makevehu
		...
		if (result) then
			...
			dbExec( exports.mysql:getConn('mta'), "INSERT INTO `vehicle_logs` (`vehID`, `action`, `actor`) VALUES (?, ?, ?) ", last_insert_id, command.." "..vehicleName.." ($"..cost.." -  "..owner..")" , adminID )

			if (hiddenAdmin==0) then
				exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(player) .. " ("..adminUsername..") spawnul "..vehicleName .. " (ID #" .. last_insert_id .. ")  "..owner.." za $"..cost..".")
				if not isFactionOwned then
					outputChatBox(tostring(adminTitle) .. " " .. getPlayerName(player) .. " spawnul "..vehicleName .. " (ID #" .. last_insert_id .. ")  "..owner.." za $"..cost..".", targetPlayer, 255, 194, 14)
				end
			else
				exports.global:sendMessageToAdmins("AdmCmd: Skrytý admin spawnul "..vehicleName .. " (ID #" .. last_insert_id .. ")  "..owner.." za $"..cost..".")
				if not isFactionOwned then
					outputChatBox("Skrytý admin spawnul "..vehicleName .. " (ID #" .. last_insert_id .. ")  "..owner.." za $"..cost..".", targetPlayer, 255, 194, 14)
				end
			end
			outputChatBox("[MAKEVEH] "..vehicleName .. " (ID #" .. last_insert_id .. ") úspěšně spawnuto "..owner..".", player, 0, 255, 0)

			if not isFactionOwned then
				outputChatBox("[MAKEVEH] $"..cost.." Bylo odebráno hráči z inventáře.", player, 0, 255, 0)
				outputChatBox("$"..cost.." bylo ti odebráno z inventáře.", targetPlayer, 0, 255, 0)
			else
				outputChatBox("[MAKEVEH] $"..cost.." bylo odebrano z hračove frakce.", player, 0, 255, 0)
			end
...
function createCivilianPermVehicle(thePlayer, commandName, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local args = {...}
		if (#args < 4) then
			outputChatBox("SYNTAX: /" .. commandName .. " [id/jmeno] [barva1 (-1 random)] [barva2 (-1 random] [Job ID -1 pro žádnej]", thePlayer, 255, 194, 14)
			outputChatBox("Job 1 = Rozvážeč baliku", thePlayer, 255, 194, 14)
			outputChatBox("Job 2 = Taxikář", thePlayer, 255, 194, 14)
			outputChatBox("Job 3 = Autobusak", thePlayer, 255, 194, 14)
		else
			...
			if not vehicleID then -- vehicle is specified as name
				...
				if vehicleEnd == #args then
					outputChatBox("Nesprávne jmeno vozidla.", thePlayer, 255, 0, 0)
					return
				else
			...
			if not (veh) then
				...
				if (insertid) then
					...
					if not addLog then
						outputDebugString("Error při vkladani logu do vozidla.")
					end
				end
			end
		end
...
function toggleLock(source, key, keystate)
	...
	elseif (lockSpam[source] == 5) then
		outputChatBox("Klidní svoje spammovani commandů!", source, 255, 0, 0)
		exports.global:sendMessageToAdmins("AdmWarn: " .. getPlayerName(source) .. " spammuje odemčení/zamknutí.")
		outputDebugString( "Možný spam od: " .. getPlayerName(source) .. " Odemčít/Zamčít")
		return
	else
				...
				--if (owner ~= -1) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						if (locked) then
							...
							if(getElementData(source, "gender")==0) then --muz
								triggerEvent('sendAme', source, "he unlocks the vehicle doors.")
							else --zena
								triggerEvent('sendAme', source, "she unlocks the vehicle doors.")
							end
						else
							...
							triggerEvent('sendAme', source, "Zamknul dveře od vozidla.")
						end
					else
						outputChatBox("(( Nemůžeš zamknout odtažene vozidla. ))", source, 255, 195, 14)
					end
				--else
					--outputChatBox("(( Nemůžeš zamknout civilní vozidla. ))", source, 255, 195, 14)
				--end
				return
			end
		end
...
function checkLock(thePlayer, seat, jacked)
	...
	if (locked) and not (jacked) then
		cancelEvent()
		outputChatBox("Dveře jsou zamčené.", thePlayer)
	end
end
...
function toggleLights(source, key, keystate)
	...
	if veh then
		if ( getElementData( veh, 'battery' ) or 100 ) > 0 then
			...
			exports.hud:sendBottomNotification( source, exports.global:getVehicleName(veh), "Baterka se vybila." )
		end
	end
end
...
function checkBikeLock(thePlayer)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424 or getElementModel(source)==431 or getElementModel(source)==437) then
		if not getElementData(thePlayer, "interiormarker") then
			outputChatBox("Vozidlo je zamčeno.", thePlayer, 255, 194, 15)
		end
		cancelEvent()
	end
end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 then
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Toto "..vehName.." je civilní vozidlo. ))" )
		elseif (faction==-1) and (owner>0) then
			...
			if ownerName and exports.integration:isPlayerTrialAdmin( thePlayer, true ) then
				exports.hud:sendBottomNotification( thePlayer, vehName, "(( Toto "..vehName.." patří " .. ownerName .. " ))" )
			end
		elseif (faction~=-1) then
			local factionName = exports.cache:getFactionNameFromId(faction) or "Neznáma frakce"
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Tohle vozidlo patří  " .. factionName .. ". ))" )
		end
	end
end
...
function sellVehicle(thePlayer, commandName, targetPlayerName, itemValue)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			outputChatBox("SYNTAX: /" .. commandName .. " [část jména / id]", thePlayer, 255, 194, 14)
			outputChatBox("Prodej hráčovi vozidlo ve kterém sedíš.", thePlayer, 255, 194, 14)
			outputChatBox("Zeptej se kupujíciho aby ti dal peníze.", thePlayer, 255, 194, 14)
		else
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or isLeader or (exports.integration:isPlayerAdmin(thePlayer, true)) then
							if getElementData(targetPlayer, "dbid") ~= getElementData(theVehicle, "owner") then
								if getElementData(theVehicle, "token") then
									outputChatBox("Tohle vozidlo nemůžeš prodat, jelikož si ho zakoupil tokenem.", thePlayer, 255, 0, 0)
									return
								end
									...
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										--if exports.integration:isPlayerTrialAdmin(thePlayer) --[[or exports['carshop-system']:isForSale(theVehicle)]] then
											...
											if query then
												...
												outputChatBox("Prodal jsi své vozidlo " .. exports.global:getVehicleName(theVehicle) .. " hráči " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
												outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " sold you a " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 0, 255, 0)
												outputChatBox("Nezapomeň nastavit park svému " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 255, 255, 0)
										...
										outputChatBox(targetPlayerName .. " ma max. počet vozidel.", thePlayer, 255, 0, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " pokoušel se ti prodat své vozidlo ale ty maš max. počet vozidel.", targetPlayer, 255, 0, 0)
									end
								else
									outputChatBox(targetPlayerName .. " nema dostatek místa pro klíčky.", thePlayer, 255, 0, 0)
									outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " pokoušel se ti prodat své vozidlo ale ty nemaš místo na klíčky.", targetPlayer, 255, 0, 0)
								end
							else
								outputChatBox("Nemůžeš prodat vozidlo sám sobě.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Tohle vozidlo neni tvé!", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Musíš byt ve vozidle.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Jsi moc daleko od " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function toggleSellExceptions (thePlayer, commandName, player)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) and player then
		...
		if getElementData(targetPlayer, "temporarySell") == true then
			...
			outputChatBox("Zakázal jsi hráči "..targetPlayerName.." použit příkaz /sell.", thePlayer)
			outputChatBox("Administrátor zakázal aby jsi použil příkaz /sell.", targetPlayer)
		else
			setElementData(targetPlayer, "temporarySell", true)
			outputChatBox("Dal jsi hráči "..targetPlayerName.." přístup na použiti příkazu /sell.", thePlayer)
			outputChatBox("Administrátor ti dal přístup na použiti příkazu /sell.", targetPlayer)
		end
	elseif not player and (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		outputChatBox("SYNTAX: /"..commandName.." [hráč] - Tohle da dočasny přistup na použiti příkazu /sell.", thePlayer)
	end
end
...
function lockUnlockInside(vehicle)
	...
	--if (owner ~= -1) then
		if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
			...
			outputChatBox("(( Nemůžeš zamknout odtažené vozidla. ))", source, 255, 195, 14)
		end
	--else
...
function lockUnlockOutside(vehicle)
	if (not source or exports.integration:isPlayerTrialAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		...
		if (isVehicleLocked(vehicle)) then
			setVehicleLocked(vehicle, false)
			triggerEvent('sendAme', source, "stiskl tlačítko pro odemknutí vozidla. ((" .. getVehicleName(vehicle) .. "))")
			exports.logs:dbLog(source, 31, {  vehicle }, "UNLOCK FROM OUTSIDE")
		else
			setVehicleLocked(vehicle, true)
			triggerEvent('sendAme', source, "stiskl tlačítko pro uzamčení vozidla. ((" .. getVehicleName(vehicle) .. "))")
			local doors = getDoorsFor(getElementModel(vehicle), -1)
			for index, doorEntry in ipairs(doors) do
				setVehicleDoorOpenRatio(vehicle, doorEntry[2], 0, 0.5)
			end
...
function fillFuelTank(veh)
	...
	if (math.ceil(currFuel)==max) then
		outputChatBox("Vůz už má plnou nadrž.", source)
	elseif (fuel==0) then
		outputChatBox("Tato nádoba na palivo je prázdná.", source, 255, 0, 0)
	elseif (engine==1) then
		outputChatBox("Nemůžete tankovat jedoucí vozidlo. Nejprve prosím vypněte motor.", source, 255, 0, 0)
	else
...
function getYearDay(thePlayer)
	...
	outputChatBox("Je rok " .. currYearday, thePlayer)
end
...
local function canParkHere( thePlayer, veh )
	if call( getResourceFromName("tow-system"), "cannotVehpos", thePlayer, veh ) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Zde není možné zaparkovat tvé vozidlo.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, HospitalCol ) and not exports.factions:isPlayerInFaction(thePlayer, 2) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Pouze ZSLS smí parkovat svá vozidla před nemocnicí.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and not exports.factions:isPlayerInFaction(thePlayer, 1) and not exports.integration:isPlayerTrialAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Pouze LSPD smí parkovat svá vozidla na Pershing Square.", thePlayer, 255, 0, 0)
	end
	return true
end
