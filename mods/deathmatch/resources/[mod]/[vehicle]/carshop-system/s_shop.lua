			...
			for k, v in ipairs( shops[dealerID]["spawnpoints"] ) do
				...
				if canPopulate2 and vehicleData then
					...
					if not (vehicle and vehBrand and vehModel and vehPrice and vehTax and vehYear and vehicle_shop_id) then
						outputDebugString("error při spawnu vozidla "..data[1])
						--next
					else
...
function refreshCarShops(thePlayer, _, fromvehmang)
	if exports.integration:isPlayerLeadAdmin(thePlayer) then
		...
		exports.global:sendMessageToAdmins("[CARSHOPS] "..staffTitle.." "..staffUsername.." Manualně obnoveny carshopy.")
		exports.logs:dbLog(thePlayer, 4, thePlayer, "RESETCARSHOP")
	else
		outputChatBox("Musíš byt admin aby jsi tohle mohl.", thePlayer, 255, 0, 0)
	end
end
...
function carshop_buyVehicle(paymentMethod)
	...
	if not exports.global:canPlayerBuyVehicle(client) then
		if isOverlayDisabled then
			outputChatBox("Maš maximalni počet vozidel!", client, 0, 255, 0)
		else
			exports.hud:sendBottomNotification(client, "Maximalni počet vozidel", "Maš maximalni počet vozidel. /stats pro detaily.")
		end
		return false
	end
	...
	if (paymentMethod == "cash") then
		if not exports.global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("Nemaš dostatek peněz..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Peníze jsou problem..", "Nemaš dostatek peněz..")
			end
			return false
		else
	...
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then
				...
				exports.hud:sendBottomNotification(client, "Peníze jsou problem..", "Nemáš dostatek peněz v bance..")
			end
			return false
		else
	...
	elseif (paymentMethod == "token") then
		if costCar <= 35000 then
			if not exports.global:takeItem(client, 263) then
				outputChatBox("Objevila se tady chyba.", client, 255, 0 ,0)
				return false
			end
		else
			outputChatBox("Nemůžeš použit token na drahší auto než 35.000.", client, 255, 0, 0)
			return false
		end
	else
		if isOverlayDisabled then
			...
			exports.hud:sendBottomNotification(client, "Ne!", "Prostě ne...")
		end
		return false
	end
	...
	if result then
		if tonumber(result["stock"]) < 1 then
			if isOverlayDisabled then
				outputChatBox("Omlouvam se tohle už je zabraný.", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Stock", "Omlouvám se bohužel vozidlo neni skladem.")
			end
			return false
		end
	end
	...
	local license = (getElementData(client, "license.car") == 1) and "" or "Nemaš řidičsky průkaz, nejezdi s tím radši."

	if isOverlayDisabled then
		outputChatBox("Gratuluju, pravě si si koupil vozidlo!", client)
		outputChatBox("Nezapomeň nastavit park, pokud ti zmizne admini ti to nevráti", client)
		outputChatBox("jednoduše nijak vozidlo nevrátiš", client)
		outputChatBox(license, client)
		outputChatBox("Uprav si popisek vozidla pomocí /ed.", client)
	else
		local content = {}
		table.insert(content, {"Nezapomeň nastavit park protože ti vozidlo zmizne."})
		table.insert(content, {license})
		table.insert(content, {"Uprav si popisek vozidla pomocí /ed."})

		exports.hud:sendBottomNotification(client, "Gratuluju, pravě si si koupil vozidlo!", content, 30)
	end
		...
		if shopname and shopnicename and vehicle_shop_id and vehYear and vehBrand and vehModel and vehPrice then
			...
			if orderedVehID and tonumber(orderedVehID) == tonumber(vehicle_shop_id) then
				if exports.global:hasItem(player, 2) then
					...
					outputChatBox("["..languagename.."] SMS from "..shopnicename..": Ahoj! objednal jsi si "..itemName.." je možne koupit za $"..exports.global:formatMoney(vehPrice)..". Come check it out!" , player, 120, 255, 80)
				end
			end
		end
	end
