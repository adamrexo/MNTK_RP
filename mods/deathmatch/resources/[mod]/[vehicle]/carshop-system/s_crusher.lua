		...
		if theVehicle and getVehicleOccupant(theVehicle) == thePlayer then -- player is driver
			if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
				...
				if getElementData(theVehicle, "requires.vehpos") then
					outputChatBox("(( Prosím zaparkuj vozidlo a vrať se. ))", thePlayer, 255, 0, 0)
				elseif price == 0 and not getElementData(theVehicle, "token") then
					...
					outputChatBox("(( Kontaktuj admina aby si prodal tve specialni vozidlo. ))", thePlayer, 255, 194, 14)
				elseif getVehicleType(theVehicle) == "Trailer" then
					outputChatBox("This " .. exports.global:getVehicleName(theVehicle) .. " nemůže byt zničeno.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Kontaktuj admina aby si prodal tve specialni vozidlo. ))", thePlayer, 255, 194, 14)
				else
				...
				outputChatBox("Nemaš to zaregistrované!", thePlayer, 255, 0, 0)
			end
		end
	end
end
	...
	if theVehicle and thePlayer then
		if getElementData( theVehicle, "owner" ) == getElementData( thePlayer, "dbid" ) then
			...
			if price and price >= 0 and dbid > 0 then
				...
				-- notify
				outputChatBox("You crushed your " .. exports.global:getVehicleName(theVehicle) .. " za $" .. exports.global:formatMoney(price) .. ".", thePlayer, 0, 255, 0)
				exports.global:sendMessageToAdmins("[VEHICLE] Maže se vozidlo #" .. dbid .. " (Zošrotovane hráčom " .. getPlayerName(thePlayer):gsub("_", " ") .. ").")
