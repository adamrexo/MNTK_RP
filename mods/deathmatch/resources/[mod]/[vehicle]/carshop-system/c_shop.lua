...
function carshop_showInfo(carPrice, taxPrice)
	...
	if isOverlayDisabled then
		...
		outputChatBox("Zmačkni F pro koupení vozu")
	else
		local content = {}
		...
		table.insert(content, {"Zmačkni F pro zakoupení!" } )
...
function carshop_buyCar(carPrice, cashEnabled, bankEnabled)
	...
	gui["lblText1"] = guiCreateLabel(20, 25, windowWidth-40, 16, "Jsi krok od toho aby jsi koupil vozidlo:", false, gui["_root"])
	...
	gui["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "Pokud zaplatíš tak nebudeš moct zažádat o vrátění peněz", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblText2"], "left", true)
	guiLabelSetVerticalAlign(gui["lblText2"], "center", true)

	gui["btnCash"] = guiCreateButton(10, 140, 105, 41, "Zaplatit penězi", false, gui["_root"])
	...
	if exports.global:hasItem(localPlayer, 263) and carPrice <= 35000 then
		guiSetText(gui["btnCash"], "Použit token")
		guiSetEnabled(gui["btnCash"], true)
	end

	gui["btnBank"] = guiCreateButton(120, 140, 105, 41, "Zaplatit bankou", false, gui["_root"])
...
function carshop_buyCar_click()
	if exports.global:hasSpaceForItem(getLocalPlayer(), 3, 1) then
		...
		if (source == gui["btnBank"]) then
			sourcestr = "bank"
		elseif guiGetText(gui["btnCash"]) == "Použit token" then
			sourcestr = "token"
		end
		triggerServerEvent("carshop:buyCar", theVehicle, sourcestr)
	else
		outputChatBox("Nemaš místo v inventáři pro klíče", 0, 255, 0)
	end
