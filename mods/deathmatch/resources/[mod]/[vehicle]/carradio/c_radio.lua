...
function ( )
	...
	if isPedInVehicle ( localPlayer ) and getElementData(localPlayer, "hide_hud") ~= "0" then
		...
		if radioID and radioID >= 0 and streams[radioID] and getVehicleType(vehicle) ~= 'BMX' and getVehicleType(vehicle) ~= 'Bike' and getVehicleType(vehicle) ~= 'Quad' then
			...
			dxDrawRectangle(left-10, top - 300, widthToDraw+80, heightToDraw+5, tocolor(0, 0, 0, 100), false)
			dxDrawText ( text, left -40, top -290 , sw, sh, tocolor ( 70, 200, 14, 255 ), 1, BizNoteFont20 or "default-bold" )
			if radioID ~= 0 and type ( song ) == "string" then
				...
				dxDrawText ( text, left - 1, top - 250, sw, sh, tocolor ( 255, 255, 255, 255 ), 1, "default-default" )
			else
				widthToDraw = size
				heightToDraw = 50
			end
