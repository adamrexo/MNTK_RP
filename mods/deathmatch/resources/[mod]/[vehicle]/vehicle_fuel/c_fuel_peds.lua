...
function requestATMInterfacePIN(theATM, atmLocationName)
	...
	if not (wPIN) then
		...
		wPIN = guiCreateWindow(x, y, width, height, "POS na  "..(atmLocationName or "neznáma oblast"), false)
			guiWindowSetSizable(wPIN, false)

		local tabPanel = guiCreateTabPanel(0.05, 0.05, 0.9, 0.85, true, wPIN)
		local tabPersonal = guiCreateTab("Prosím zadej PIN", tabPanel)
		...
		Label_Error = guiCreateLabel(20,90,220,30,"Prosím zadej první 4 čislice PINu",false,tabPersonal)
...
function respondToATMInterfacePIN(Label_Error_Msg, r,g,b, action, otherInfo)
	...
	if action == "cardRemoved" then
		exports.hud:sendBottomNotification(localPlayer, "Point of Sale Machine", "Bankomat nefunguje, karta byla vytažena.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "failedLessThan3" then
		exports.hud:sendBottomNotification(localPlayer, "Point of Sale Machine", "Přístup zamítnut. PIN byl špatně zadán.")
		guiSetEnabled(bEnter, true)
	elseif action == "locked" then
		exports.hud:sendBottomNotification(localPlayer, "Point of Sale Machine", "Čislo karty '"..otherInfo.."' je zamknutá. Kontaktujte banku.")
		guiSetEnabled(bEnter, true)
	elseif action == "success" then
		exports.hud:sendBottomNotification(localPlayer, "Point of Sale Machine", "Přistup povolen!")
		closeATMInterfacePIN()
		local option = 2
