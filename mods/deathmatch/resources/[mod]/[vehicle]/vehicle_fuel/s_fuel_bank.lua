...
function checkPINCode(enteredCode)
	...
	if not foundAnATMCard then
		exports.hud:sendBottomNotification(source,"Bankomat nefunguje. Karta kterou si vložil magicky zmizla!")
		return false
	end
	...
	if check["card_locked"] == "1" then
		triggerClientEvent(source, "fuel:respondToATMInterfacePIN", source, "ERROR: Kartu neni možne použit.", 255,0,0, "locked", cardNumber )
		return false
	end
	
	triggerClientEvent(source, "fuel:respondToATMInterfacePIN", source, "Přístup povolen!", 70,255,14, "success" )
