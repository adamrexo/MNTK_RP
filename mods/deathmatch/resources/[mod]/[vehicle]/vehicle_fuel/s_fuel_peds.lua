...
function startTalkToPed ()
	...
	if not (isPedInVehicle(thePlayer)) or (isPedInVehicle(thePlayer) and getVehicleType(getPedOccupiedVehicle(thePlayer)) == "BMX") then
		processMessage(thePed, "Dobrý den, co pro vás můžu udělat?.")
 		setConvoState(thePlayer, 3)
		local responseArray = { "Můžete mi naplnit kanistr??", "Nic..", "Nemáte práci pro mě", "Líbi se mi váš oblek." }
		...
		if (exports.vehicle:isVehicleWindowUp(theVehicle)) then
			outputChatBox("Musíš prvně dat dolu okno, aby jsi mohl s někym mluvit", thePlayer, 255,0,0)
			return
		end
		-- processMeMessage(thePed, "leans against " .. getPlayerName(thePlayer):gsub("_"," ") .. "'s vehicle.", thePlayer )
		triggerEvent('sendAme', thePed, "se opřel o " .. getPlayerName(thePlayer):gsub("_"," ") .. "' vozidlo.")
		processMessage(thePed, "Jak vam můžu pomoct?")
 		setConvoState(thePlayer, 1)
		local responseArray = { "Můžete mi natankovat?.", "Ne, děkuji.", "Nemáte cigaretu pro mě?", "Neopírejte se o mé vozidlo." }
		triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
	end
end
...
function talkToPed(answer, answerStr)
	...
	if (convState == 1) then -- "Hey, how could I help you?"
		...
		if (languageSkill < 60) or (currLang ~= 1) then
			processMessage(thePed, "Omlouvám se, mluvím pouze česky.")
		...
		if (answer == 1) then -- "Ehm, fill my tank up, please."
			...
			if (getElementData(theVehicle, "engine") == 1) then
				processMessage(thePed, "Můžete prosím vypnout motor?")
				local responseArray = { "Jistě, neni problém.", "Nemůžete to udělat když je motor nastartovaný?", "Co!?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				setConvoState(thePlayer, 2)
				return
			elseif getElementData(theVehicle, 'fuel') > (getMaxFuel(getElementModel(theVehicle))-1) then
				processMessage(thePed, "Připadá mi to plný.")
			else
				processMessage(thePed, "Jistě, čím chcete platit?")
				if getATMCardFromATMMachine(thePlayer) then
					local responseArray = { "Penězi prosím!", "Použijem bankovní převod.", "Prominte, co?" }
					triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				else
					local responseArray = { "Penězi prosím!", false, "Sorry what?" }
					triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
				end
		...
		elseif (answer == 2) then -- "No thanks."
			processMessage(thePed, "Dobrá, přijdte když budete potřebovat natankovat.")
			setConvoState(thePlayer, 0)
		elseif (answer == 3) then -- "Do you have a sigarette for me?"
			processMessage(thePed, "Um.. ne, můžete kouknout 14/7.")
			setConvoState(thePlayer, 0)
		elseif (answer == 4) then -- stop leaning against my car
			processMessage(thePed, "Dobře, dobře. Vemte to lehce.")
			--processMeMessage(thePed, "pushes himself up again, standing on his feet.", thePlayer )
			triggerEvent('sendAme', thePed, "postavil se na špičky")
			processMessage(thePed, "Mam to natankovat nebo ne?.")
			local responseArray = {  "Jděte do toho.", "No, not anymore." }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 1)
		end
	elseif (convState == 2) then -- "Could you please turn your engine off?"
		if (answer == 1) then -- "Sure, no problemo." / "Ok, okay.."
			...
			triggerEvent('sendAme', thePlayer, "vypnul vozidlo.")
			...
			processMessage(thePed, "Dobře děkuji, jak by jste rád zaplatili?")
			if getATMCardFromATMMachine(thePlayer) then
				local responseArray = { "Penězi prosím!", "Použiju bankovní převod.", "Co!?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			else
				local responseArray = { "Penězi prosím", false, "Co!?" }
				triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			end
			setConvoState(thePlayer, 4)
		elseif (answer == 2) then -- "Can't you do it with the engine running?"
			...
			processMessage(thePed, "Ehm... Ne, nechci umřit. Vypněte to.?")
			local responseArray = {  "Jděte do toho.", false, false, "Dobře, vypnu to.."  }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 2)
		elseif (answer == 3) then -- "Eh, WHAT?"
			processMessage(thePed, "Ptal jsem se, mohli by jste vypnout vozidlo?")
			local responseArray = {  "Dobře", false,false, "Ugh, ne."  }
			triggerClientEvent(thePlayer, "fuel:convo", thePed, responseArray)
			setConvoState(thePlayer, 2)
		elseif answer == 4 then -- "Ugh, shut up then." / "Ugh, no."
			processMessage(thePed, "Dobře.. vemte to lehce a zmizte.")
			setConvoState(thePlayer, 0)
		end
	elseif (convState == 3) then
		if answer == 1 then -- Could you fill my fuelcan?
			if (exports.global:hasItem(thePlayer, 57)) then
				...
				triggerEvent('sendAme', thePed, "Dal hadici do vozidla a pustil ji.")
				triggerEvent('sendAme', thePed, "natahuje hadici a pomalu tankuje.")
				setTimer(pedWillFillFuelCan, 3500, 1, thePlayer, thePed)
			else
				processMessage(thePed, "Potřebujete kanistr pro tohle.")
				setConvoState(thePlayer, 0)
			end
		elseif answer == 2 then -- No thanks
			processMessage(thePed, "Dobře, mějte hezký den.")
			setConvoState(thePlayer, 0)
		elseif answer == 3 then -- do you have a cigarette for me?
			processMessage(thePed, "Koukněte do 24/7.")
			setConvoState(thePlayer, 0)
		elseif answer == 4 then -- I like your suit
			processMessage(thePed, "Em... děkuji tedy.")
			setConvoState(thePlayer, 0)
		end
