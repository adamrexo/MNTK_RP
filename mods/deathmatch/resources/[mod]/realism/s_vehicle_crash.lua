...
function seatbelt(thePlayer)
	if getPedOccupiedVehicle(thePlayer) and getElementData(thePlayer, "realinvehicle") == 1 then
		...
		if (getVehicleType(theVehicle) == "BMX") or (noBelt[getElementModel(theVehicle)] and getVehicleOccupant(theVehicle, 0) ~= thePlayer) then
			outputChatBox("Zvláštní... Na tomto vozidle není žádný bezpečnostní pás!", thePlayer, 255, 0, 0)
		else
			local helmet = false
			if (getElementData(thePlayer, "seatbelt") == true) then
				if getVehicleType(theVehicle) ~= "Bike" then
					...
					outputChatBox("Rozepnul si bezpečnostní pás.", thePlayer, 255, 0, 0)
					triggerEvent("sendAme", thePlayer, "si rozepnul bezpečnostní pás.")
				elseif getVehicleType(theVehicle) == "Bike" then
					...
					if helmet then
						triggerEvent("sendAme", thePlayer, "si sundava helmu.")
				...
				if getVehicleType(theVehicle) ~= "Bike" then
					...
					outputChatBox("Zapnul si bezpečnostní pás.", thePlayer, 0, 255, 0)
					triggerEvent("sendAme", thePlayer, "si zapnul bezpečnostní pás")
				elseif getVehicleType(theVehicle) == "Bike" then
					...
					if helmet then
						exports.anticheat:changeProtectedElementDataEx(thePlayer, "seatbelt", true, true)
						triggerEvent("sendAme", thePlayer, "si nasadil helmu.")
					end
				end
			end
		end
