...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		wRanks = guiCreateWindow(sX, sY, wX, wY, "Hodnosti", false)
		guiWindowSetSizable(wRanks, false)
		guiSetAlpha(wRanks, 1.0)
		-- Labels
		ranksLabel1 = guiCreateLabel(9, 27, 172, 15, "Frakčne hodnosti", false, wRanks)
		...
		ranksLabel2 = guiCreateLabel(288, 27, 142, 15, "Frakčne práva", false, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Tvoja hodnosť obsahuje nevhodné charaktery, pozri či nemaš nejaké nedovolene znaky ako '@.;", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("Tvoje výplaty niesu zapísane ako čísla.", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btRenameRank()
	...
	wRenameRank = guiCreateWindow(sX, sY, wX, wY, "Přejmenovat hodnost", false)
	guiWindowSetSizable(wRenameRank, false)
	-- Label
	lRenameRank1 = guiCreateLabel(-18, 28, 277, 15, "Změnit '"..currentRank.."' hodnost na:", false, wRenameRank)
...
function btRemoveRank()
	...
	wRemoveRank = guiCreateWindow(660, 366, 285, 138, "Smazaní hodnosti", false)
	guiWindowSetSizable(wRemoveRank, false)
	-- Labels
	local row = guiGridListGetSelectedItem(ranksGridlist)
	local rankName = guiGridListGetItemText(ranksGridlist, row, 1)
	lRemoveRank1 = guiCreateLabel(26, 24, 225, 30, "Jsi si jistej že chceš smazat \n'"..rankName.."' hodnost?", false, wRemoveRank)
	...
	lRemoveRank2 = guiCreateLabel(10, 58, 257, 31, "Všichni členové dostanou \n'"..def_rank.."' jako hodnost.", false, wRemoveRank)
...
function setRankInfo(rankTbl, rankperms, perms, wageTable)
	...
	if not isElement(ranksLabel5) then
		ranksLabel5 = guiCreateLabel(0, 0, 337, 15, "Vyber rank pro upravení", false, ranksScrollPane)
...
function selectRankOffList()
	...
	guiSetText(ranksLabel5, "Upravovaní práv pro '"..ranks[row+1][2].."'")
end
...
function applyPermissions()
	...
	if (not row or row == -1) then 
		outputChatBox("Prvně vyber hodnost.", 255, 100, 100)
	return end
	local rankID = ranks[row+1][1]
...
function updateRankOrder()
	...
	if (source == btRanksMvUp or source == btRanksMvDn) then
		local row = guiGridListGetSelectedItem(ranksGridlist)
		if (not row or row == -1) then 
			outputChatBox("Prvně vyber hodnost.", 255, 125, 0)
		return end
...
function renameRank()
	...
	if (rankName == "") then
		outputChatBox("Napíš hodnost.", 255, 100, 100)
		return
	end
