...
function callbackQuitFaction(factionID)
	...
	local response = dbQuery(mysql:getConn("mta"), "SELECT `character_id` FROM `characters_faction` WHERE `faction_id` = ? AND `faction_leader` = 1;", factionID)
	local factionLeaders, numOfLeaders = dbPoll(response, -1)
	dbFree(response)
...
addEventHandler("faction-system.saveNewRank", root, 
	function(playerName, oldRank, newRank, factionID)
		...
		local rankold = oldRank
		local ranknew = newRank
		...
		if (client == nil) then
			client = exports.global:getPlayerFromCharacterID(charID)
		end
		...
		if (rankold == "Leader" or rankold == "Leader Rank") then
			local response = dbQuery(mysql:getConn("mta"), "SELECT `character_id` FROM `characters_faction` WHERE `faction_id` = ? AND `faction_leader` = 1;", fID)
			local factionLeaders, numOfLeaders = dbPoll(response, -1)
			dbFree(response)
		end
		...
		if (ranknew == "Leader" or ranknew == "Leader Rank") then
			local response = dbQuery(mysql:getConn("mta"), "SELECT `faction_id` FROM `characters_faction` WHERE `character_id` = ? AND `faction_leader` = 1;", charID)
			local factionsWhereIsLeader, numOfLeaderRank = dbPoll(response, -1)
			dbFree(response)
			if (numOfLeaderRank < 2) then
				dbExec(exports.mysql:getConn("mta"), "UPDATE `characters_faction` SET `faction_leader` = ? WHERE `character_id` = ? AND `faction_id` = ?", 1, charID, fID)
			else
...
addEventHandler("faction-system.getFactionPlugins", root, function(factionID)
	iprint("getting faction plugins")
	local factionID = tonumber(factionID)
