	...
	if res then
		...
		-- if creating new faction.
			...
			if id and tonumber(id) then
				...
				for i=1,2 do
					if i == 1 then
						local qh3 = dbQuery( exports.mysql:getConn('mta'), "INSERT INTO faction_ranks SET faction_id=?, name='Leader', permissions=?, isDefault='0', isLeader='1', wage='0'", data.id, permissions)
						local res, nums, rid1 = dbPoll( qh3, 10000 )
						local rankID = tonumber(rid1)
						FactionRanks[rankID] = {}
						FactionRanks[rankID]["name"] = "Leader"
						...
						factionRanks[rankID] = "Leader"
						factionWages[rankID] = 0
						rank_order = rank_order..rankID..","
						dbFree( qh3 )

					elseif i == 2 then	
						local qh4 = dbQuery( exports.mysql:getConn('mta'), "INSERT INTO faction_ranks SET faction_id=?, name='Default', permissions='', isDefault='1', isLeader='0', wage='0'", data.id)
						local res, nums, rid2 = dbPoll( qh4, 10000 )
						local rankID = tonumber(rid2)
						FactionRanks[rankID] = {}
						FactionRanks[rankID]["name"] = "Default"
						FactionRanks[rankID]["permissions"] = ""
						FactionRanks[rankID]["isDefault"] = 1
						FactionRanks[rankID]["isLeader"] = 0
						FactionRanks[rankID]["faction_id"] = data.id
						table.insert(RanksByFaction[data.id], tonumber(rankID))
						factionRanks[rankID] = "Default"
						factionWages[rankID] = 0
						rank_order = rank_order..rankID..","
						dbFree( qh4 )
					end
...
addEvent( 'factions:usercreate', true )
addEventHandler( 'factions:usercreate', resourceRoot, function( player , name)

	iprint(player)
	iprint(name)
	local playername = getPlayerName(player)
	local playerID = exports.global:getCharacterIDFromName(playername)
	local qh = dbQuery( exports.mysql:getConn('mta'), "SELECT * FROM characters_faction WHERE character_id = ? AND faction_leader = 1;", playerID)
	local res, nums = dbPoll( qh, -1 )
	iprint(nums)
	if (nums < 2) then
		...
		if num_factions >= 5 then
			outputChatBox("Už jsi v maximálním počtu frakcí.", client, 255, 0, 0)
			return
		end
		iprint("create fr")
		money = exports.global:getMoney(player)
		if(money >= 500000) then
			exports.global:takeMoney(player,1)
			local data = {}
			data.name = name
			data.type = 5
			data.max_interiors = 20
			data.max_vehicles = 40
			data.free_custom_ints = 0
			data.free_custom_skins = 0
			data.before_tax_value = 0
			data.free_wage_amount = 0
			...
			if id and tonumber(id) then
				...
				local defaultRank = getDefaultRank(data.id)
			
				if dbExec(exports.mysql:getConn("mta"), "INSERT INTO characters_faction SET faction_leader = 0, faction_id = ?, faction_rank = ?, faction_phone = NULL, character_id=?", data.id, defaultRank, getElementData(client, "dbid")) then
					local max = 0
					...
					factionInfo[data.id] = { rank = defaultRank, leader = false, phone = nil, perks = { }, count = max+1 }
					exports.anticheat:changeProtectedElementDataEx(client, "faction", factionInfo, true)

					triggerEvent("duty:offduty", client)
					exports.logs:dbLog(thePlayer, 4, { client, theTeam }, "SET TO FACTION")

				triggerEvent("faction-system.saveNewRank", resourceRoot, getPlayerName(client), "Default", "Leader", tostring(data.id) )
				return
				else
					return
				end
			...
			outputChatBox("Hráč u sebe nemá dostatek peněz.", client, 255, 125, 0)
			return
		end

	else
		outputChatBox("Hráč nemůže být lídr ve více než 2 frakcích.", client, 255, 125, 0)
		return
	end
end)
