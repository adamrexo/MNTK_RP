...
local user_createNPC = createPed( 141, 356.2978515625, 178.37109375, 1008.3762207031 )
setPedRotation( user_createNPC, 300 )
setElementDimension( user_createNPC, 109 )
setElementInterior( user_createNPC , 3 )
setElementData( user_createNPC, "talk", 1, false )
setElementData( user_createNPC, "name", "Lorin Noel", false )
--setPedAnimation ( jessie, "INT_OFFICE", "OFF_Sit_Idle_Loop", -1, true, false, false )
setElementFrozen(user_createNPC, true)
...
function showFactionList( factions )
	...
	if not fGUI.window[1] and not isElement( fGUI.window[1] ) then
		...
		fGUI.button[1] = guiCreateButton(674, 552, 116, 38, "Zavřít", false, fGUI.window[1])
		fGUI.button[2] = guiCreateButton(10, 552, 116, 38, "Vytvořit frakci", false, fGUI.window[1])
		fGUI.button[3] = guiCreateButton(136, 552, 116, 38, "Upravit frakci", false, fGUI.window[1])
		fGUI.button[4] = guiCreateButton(262, 552, 116, 38, "Tabulka členů", false, fGUI.window[1])
		fGUI.button[5] = guiCreateButton(388, 552, 116, 38, "Smazat frakci", false, fGUI.window[1])
		fGUI.button[6] = guiCreateButton(514, 552, 116, 38, "Obnovit", false, fGUI.window[1])
		...
		addEventHandler( 'onClientGUIClick', fGUI.window[1], function()
			...
			elseif source ==  fGUI.button[3] and fGUI.gridlist[1]  then
				...
				if row ~= -1 and col ~= -1 then
					...
					outputChatBox( "Prosím, prvně vyber item z možnosti výběru.", 255, 0, 0 )
				end
			elseif source == fGUI.button[4] then
				...
				if row ~= -1 and col ~= -1 then
					...
					exports.global:playSoundError()
					outputChatBox( "rosím, prvně vyber item z možnosti výběru.", 255, 0, 0 )
				end
	...
	if not factions then
		if not fGUI.label[1] then
			...
			fGUI.label[1] = guiCreateLabel ( 0, 0, 1, 1, "Ziskavani informaci ze serveru...", true, fGUI.window[1] )
		...
		fGUI.gridlist.colName = guiGridListAddColumn(fGUI.gridlist[1], "Jmeno frakce", 0.23)
		fGUI.gridlist.colPlayers = guiGridListAddColumn(fGUI.gridlist[1], "Počet online členu", 0.1)
		fGUI.gridlist.colType = guiGridListAddColumn(fGUI.gridlist[1], "Typ", 0.1)
		fGUI.gridlist.colInts = guiGridListAddColumn(fGUI.gridlist[1], "Interiery", 0.07 )
		fGUI.gridlist.colVehs = guiGridListAddColumn(fGUI.gridlist[1], "Vozidla", 0.07 )
		fGUI.gridlist.colIntPerk = guiGridListAddColumn(fGUI.gridlist[1], "Zdarma vlastní interier", 0.08)
		fGUI.gridlist.colSkinPerk = guiGridListAddColumn(fGUI.gridlist[1], "Zdarma vlastní skiny", 0.08)
		fGUI.gridlist.colBeforeTax = guiGridListAddColumn(fGUI.gridlist[1], "Částka před daněma", 0.14)
		fGUI.gridlist.colFreeWage = guiGridListAddColumn(fGUI.gridlist[1], "Daně po té co je naučtovano na frakci", 0.20)
...
addCommandHandler( "frakce", showFactionListCmd, false, false )
...
function editFaction( fact_id )
    ...
    editFact.window[1] = guiCreateWindow(155, 453, 385, 300, fact_id and "Upravit frakci" or "Vytvořit novou frakci", false)
    guiWindowSetSizable(editFact.window[1], false)
    exports.global:centerWindow( editFact.window[1] )
    togWindow( fGUI.window[1], false )

    editFact.label[1] = guiCreateLabel(20, 26, 97, 29, "Jmeno frakce:", false, editFact.window[1])
	...
	editFact.label[7] = guiCreateLabel(20, 162, 140, 29, "Daně předtím než je frakce zaučtovana:", false, editFact.window[1])
    ...
    editFact.combobox[1] = guiCreateComboBox(120, 100, 245, 29, "Prosím, prvně vyber typ frakce", false, editFact.window[1])
...
addEvent( 'factions:showusercreate', true )
addEventHandler( 'factions:showusercreate', getRootElement(), function ()
	setElementData(localPlayer, "exclusiveGUI", true, false)
	usercreate_win = guiCreateWindow(155, 453, 385, 300, "Vytvoření frakce", false)
	guiSetInputMode("no_binds_when_editing")
    guiWindowSetSizable(usercreate_win, false)
    exports.global:centerWindow( usercreate_win )
	fname = guiCreateLabel(20, 26, 97, 29, "Faction Name:", false, usercreate_win)
    guiLabelSetVerticalAlign(fname, "center")
    editFact.edit.name = guiCreateEdit(117, 26, 246, 29,"", false, usercreate_win)
    guiEditSetMaxLength( editFact.edit.name, 200 )
	editFact.button[1] = guiCreateButton(179, 250, 88, 40, "Cancel", false, usercreate_win)
    editFact.button[2] = guiCreateButton(277, 250, 88, 40, "Submit", false, usercreate_win)

	addEventHandler( 'onClientGUIClick', usercreate_win, function()
    	if source == editFact.button[1] then
			setElementData(localPlayer, "exclusiveGUI", false, false)
    		destroyElement( usercreate_win )
			editFact.window[1] = nil
    	elseif source == editFact.button[2] then
    		local factionname = guiGetText( editFact.edit.name )

    		if string.len( factionname ) < 3 then
    			exports.global:playSoundError()
    			return not outputChatBox( "Jméno frakce musí být delší než 3 znaky.", 255, 0, 0 )
    		else
    			triggerServerEvent( 'factions:usercreate', resourceRoot, getLocalPlayer(), factionname )
				setElementData(localPlayer, "exclusiveGUI", false, false)
    			destroyElement( usercreate_win )
				editFact.window[1] = nil
    		end
