...
function showChemistrySet()
	if not (wItems) then
		if not (wChemistrySet) then
			...
			wChemistrySet = guiCreateWindow(x, y, width, height, "Chemická sada", false)
			...
			colChemName = guiGridListAddColumn(gChemicals, "Název", 0.855)
			...
			-- buttons
			bMixItems = guiCreateButton(0.05, 0.91, 0.7, 0.15, "Zpracovat výběr", true, wChemistrySet)
			addEventHandler("onClientGUIClick", bMixItems, mixItems, false)
			guiSetEnabled(bMixItems, false)
			
			bChemClose = guiCreateButton(0.8, 0.91, 0.15, 0.15, "Zavřít", true, wChemistrySet)
...
function createRandomPeds()
	if(math.random(1,30)==10)then
		if (#peds<15) then
			...
			z = (getGroundPosition(x+rand1, y+rand2, z+20)+0.5)
			
			if not(rand1==0 or rand2==0)then
				local theped = createPed(math.random(1,299), x+rand1, y+rand2, z, math.random(0,359))
				table.insert(peds, theped)
			end
		else
			local randnum = math.random(1,#peds)
			if(peds[randnum])then
				destroyElement(peds[randnum])
				peds[randnum] = nil
			end
		end
	end
...
addCommandHandler("psst", resetDrug4Effect, false, false)
...
function doDrug6Effect()
	if not (drug6effect) then
		...
		addEventHandler("onClientPlayerDamage", getLocalPlayer(), cancelEvent)
		--setGameSpeed(0.5)
		drug6timer2 = setTimer(doRandomMessage, 22000, 5)
	end
end

local drugMessages = {
	"Debile!", "Kááámo, ty lítáš!", "Dej mi drogy!", "Hele, pohoda, kámo!", "Hele vole, kde máš nohy?",
	"Chceš tancovat?", "Co je to za sračku?", "Zavolám policii.", "Hale tančící drak.", "Potřebuji kafe.",
	"Vidíš ty zeblitý dveře?", "Ty krávo to je pěkná duha.", "Jednou bych chtěl bejt želva.", "Wauu, tak ten mravenec je drsnej.",
	"Brácho dáš mi kilo?", "Pepo, je oběd!", "Támhle vidím černou díru.", "Dal bych si žraloka na smetaně.",
	"Už jsi viděl ty běhající krabičky?", "Hm, co by moje máma řekla na to, že jsem dráček?", "Život je krutej!"
}
	...
	for key, value in ipairs(players) do
		if (value~=localPlayer) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<15) then
				...
				outputChatBox(charname .. " říká: " .. drugMessages[rand2], 255, 255, 255)
				break
			end
		end
	end
...
addEventHandler("accounts:characters:spawn", getRootElement(), resetAllDrugs)
