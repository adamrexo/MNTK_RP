function addUpgrade( source, vehicle, itemSlot, itemID, itemValue )
	...
	for key, value in ipairs(old_upgrades) do
		if value == itemValue then
			outputChatBox("Vylepšeni už je přidáno.", source, 255, 0, 0)
			return
	...
	if getElementData(vehicle, "job") ~= 0 or getElementData(vehicle, "owner") == -2 then
		outputChatBox("Nemůžeš nainstalovat vylepšení do civilního vozidla!", source, 255, 0, 0)
		return
	end
	...
	if takeItemFromSlot(source, itemSlot) then
		if addVehicleUpgrade(vehicle, itemValue) then
			...
			exports.global:sendLocalMeAction(source, "přidal " .. getItemDescription(itemID, itemValue) .. " do jeho " .. exports.global:getVehicleName(vehicle).."("..getVehicleName(vehicle) .. ").")
			...
			outputChatBox("Tohle vylepšeni neni kompatibilní s " .. exports.global:getVehicleName(vehicle) .. ".", source, 255, 194, 14)
			giveItem( source, itemID, itemValue )
		end
	else
		outputChatBox("Selhani při braní itemu.", source, 255, 194, 14)
	end
end
...
function moveUpgradeFromElement( upgrade )
	...
	if hasSpaceForItem( client, 114, upgrade ) then
		if removeVehicleUpgrade( source, upgrade ) then
			...
			outputChatBox( "Vylepšení pro vozidlo neexistuje?", client, 255, 0, 0 )
		end
	else
		outputChatBox( "Tvůj inventář je plný.", client, 255, 0, 0 )
	end
	triggerClientEvent( client, "finishItemMove", client )
end
