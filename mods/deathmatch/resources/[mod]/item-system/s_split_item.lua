	...
	--if exports.integration:isPlayerTrialAdmin(player) then
		...
		if itemID and amount then
			if (itemID%1 ~= 0) or (amount%1 ~= 0) then
				outputChatBox("Item ID musí byt platne, př. 1,2,3,..", player, 255, 0, 0, false)
				return false
			end
			if itemID > 0 and splittableItems[itemID] then
				...
				if isPlayerHasItem then
					...
					if not itemValue2 then
						outputChatBox("Tvůj item drogy je zabugovany. Prosím kontaktuj admina pomocí F1", player,255,0,0)
						return false
					end
					...
					if amount > 0 then
						if amount <= itemValue2 then
							...
							if exports.global:takeItem(player, itemID, itemValue) and giveItem( player, itemID, amount, false , true) and giveItem( player, itemID, itemRemaining,false ,true) then
								...
								outputChatBox("Něco se nepodařilo. Prosím kontakuj admina pomocí F1.", player, 255, 0, 0, false)
								return false
							end
						else
							outputChatBox("Množství nemůže byt větší než maš v inventáři.", player, 255, 0, 0, false)
						end
					else
						outputChatBox("Množství musí byt nad nulou.", player, 255, 0, 0, false)
					end
				else
					outputChatBox("Nemaš danou věc v intentáři. Napíš '/splits' pro tabulku rozdělení věci.", player, 255, 0, 0, false)
				end
			else
				outputChatBox("ID '" .. itemID .. "' neni možné rozdělit. Napiš'/splits' pro tabulku rozdělení věci.", player, 255, 0, 0, false)
			end
		else
			outputChatBox("SYNTAX: /" .. cmd .. " [Item ID] [Množství]", player, 255, 194, 14, false)
			outputChatBox("Type '/splits' pro tabulku rozdělení věci.", player, 255, 194, 14, false)
		end
	--end
end
...
function listSplittable(thePlayer, commandName)
	outputChatBox("Tabulka rozdělení věcí & ID:",thePlayer, 255, 194, 14)
	for itemID = 1, 150 do
