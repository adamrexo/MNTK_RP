...
function dropItem(itemID, x, y, z, ammo, keepammo)
	if disableCanDropPick then
		outputChatBox("Pokládání je současně deaktivováno. Toto může být chyba, informujte Administrátory.", source, 255, 0, 0)
	...
	if not ammo then
		...
		if not exports.global:isStaffOnDuty(source) then
			if ((itemID >= 31) and (itemID <= 43)) or itemBannedByAltAltChecker[itemID] then 
				...
				if hoursPlayedFrom < 10 then
					outputChatBox("Musíš mít nahráno alespoň 10 hodin, aby jsi položil "..getItemName( itemID )..".", source, 255, 0, 0)
					triggerClientEvent( source, "finishItemDrop", source )
					return 
				end
			end
		...
		if ( itemID == 115) then -- Weapons
			...
			if (tonumber(weaponDetails[2]) and tonumber(weaponDetails[2]) == 2)  then -- /duty
				outputChatBox("Nemůžeš položit svoji služební zbraň.", source, 255, 0, 0)
				weaponBlock = true
			end
		...
		elseif ( itemID == 116) then -- Ammo
			...
			if (checkString == " (D)")  then -- /duty
				outputChatBox("Nemůžeš položit svoje služební náboje.", source, 255, 0, 0)
				weaponBlock = true
			end
		end
	
		--if itemID == 60 or itemID == 137 or itemID == 138 or itemID == 175 and not exports.integration:isPlayerAdmin(source) then
		if itemID == 60 or itemID == 137 or itemID == 138 and not exports.integration:isPlayerAdmin(source) then
			outputChatBox( "Tento předmět nemůže být položen.", source, 255, 0, 0 )
		elseif ( itemID == 81 or itemID == 103 ) and dimension == 0 then
			outputChatBox( "Tento předmět musíš položit v interiéru.", source )
		...
		elseif itemID == 139 then
			...
			elseif itemID == 134 then
				if not exports.global:takeMoney(source, itemValue) then
					outputDebugString(getPlayerName(source) .. ' (' .. getPlayerSerial(source) .. ') se pokusil položit peníze, které nemá.')
			...
			if insert then
				...
				elseif (itemID==178) then
					...
					name = ("kniha s názvem ".. yup[1].." od ".. yup[2])
				end
				
				if itemID == 134 then
					outputChatBox("Položil jsi " .. exports.global:formatMoney(itemValue) .. "$.", source, 255, 194, 14)
				elseif (itemID == 80) and tostring(itemValue):find( ":" ) then
					outputChatBox("Položil jsi " .. name .. ".", source, 255, 194, 14)
				else
					outputChatBox("Položil jsi " .. string.lower(getItemName( itemID, itemValue )) .. ".", source, 255, 194, 14)
				end
				...
				if itemID == 134 then
					triggerEvent('sendAme', source, "položil " .. exports.global:formatMoney(itemValue) .. "$.")
				elseif (itemID==80 or itemID==178) and tostring(itemValue):find( ":" ) then
					triggerEvent('sendAme', source, "položil " .. name .. ".")
				else
					triggerEvent('sendAme', source, "položil " .. getItemName( itemID, itemValue ) .. ".")
				end
			end
		end
	else
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("Nemůžeš pokládat zbraně, když jsi ve službě.", source, 255, 0, 0)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("Nemůžeš položit tento sprej.", source, 255, 0, 0)
		else
			...
			outputChatBox("Položil jsi " .. ( getWeaponNameFromID( itemID ) or "neprůstřelnou vestu" ) .. ".", source, 255, 194, 14)
			...
			if query then
				...
				triggerEvent('sendAme', source, "položil "..getItemName(-itemID)..".")
...
function pickupItem(object, leftammo)
	...
	if not canPickup(source, object) then
		outputChatBox("Pouze Administrátor může vzít tento předmět.", source, 255, 0, 0)
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.global:isStaffOnDuty(source) then
			outputChatBox("Musíš mít nahráno alespoň 10 hodin pro sebrání "..getItemName( itemID )..".", source, 255, 0, 0)
			exports.anticheat:changeEld(object, "transfering", nil)
			return false
		end
		...
		if creator ~= picker then
			...
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("Přesun předmětů z postavy na postavu je přísně zakázán! Tvůj pokus byl nahlášen Administrátorovi. ", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("[ANTI-ALTtoALT]: Detekován nelegální přesun předmětů na účtu '"..getElementData(source, "account:username").."'.")
				exports.global:sendMessageToAdmins("[ANTI-ALTtoALT]: Tento hráč se pokusil přehodit předmět '"..getItemName( itemID ).."' z postavy na postavu.")
	...
	if itemID == 115 then
		if isThisGunDuplicated(itemValue, source) then
			...
			outputChatBox("Zbraň, kterou jsi sebral byla duplikována bugem a byla z tohoto důvodu odstraněna, omlouváme se.", source, 255,0,0)
			return false
		end
	end
	...
	if itemID == 134 then -- money
		...
		outputChatBox("Sebral jsi " .. exports.global:formatMoney(itemValue) .. "$.", source, 255, 194, 14)
		triggerEvent('sendAme', source, "zvedl " .. exports.global:formatMoney(itemValue) .. "$.")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
		giveItem(source, itemID, itemValue)
		triggerEvent('sendAme', source, "zvedl " .. getItemDescription( itemID, itemValue ) .. ".")
	else
		giveItem(source, itemID, itemValue)
		outputChatBox("Sebral jsi " .. getItemName( itemID, itemValue ) .. ".", source, 255, 194, 14)
		triggerEvent('sendAme', source, "zvedl " .. getItemName( itemID, itemValue ) .. ".")
	end
