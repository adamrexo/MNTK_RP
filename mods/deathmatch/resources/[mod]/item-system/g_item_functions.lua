...
function getItemName(id, value)
	...
	if id == -100 then
		return "Neprůstřelná vesta"	
	elseif id == -46 then -- MTA Client bug
		return "Padák"
	--[[elseif id == 259 then -- Tablet - Made by RecuvaPumDEV
		triggerClientEvent(getLocal)]]--
	elseif id < 0 then
	...
	elseif ( id == 90 or id == 171 or id == 172 ) and value then
		...
		if itemValue[1] and itemValue[2] then
			local customName = tostring(itemValue[1]).." helma"
			return customName
		else
			return g_items[id][1]
		end
	...
	elseif(id == 175) then --poster
		if value and not tonumber(value) then
			local itemExploded = explode(";", value)
			local name = "Plakát - "..tostring(itemExploded[1].."")
			return name
		else
			return g_items[id][1]
		end
	...
	if i then
		...
		elseif id == 150 then--ATM card
			...
			if tonumber( itemExploded[2] ) > 0 then
				return "Číslo karty: '"..itemExploded[1].."', Majitel: '"..tostring(exports.cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'"
			else
				return "Číslo karty: '"..itemExploded[1].."', Majitel: '"..tostring(exports.cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'"
			end
		elseif id == 152 then--ID card
			local itemExploded = explode(";", value)
			return "Občanský průkaz se jménem "..tostring(itemExploded[1]):gsub("_", " ")..", fotkou a několika základními informacemi."
		elseif id == 178 then
			local yup = split(value, ":")
            return yup[1] .." od ".. yup[2]
		else
			return desc:gsub("#v",value)
		end
	end
...
itemBannedByAltAltChecker = { 
	[2] = true, --CELLPHONE
	[259] = true, --Tablet made by RecuvaPumDEV
	[3] = true, --VEHICLE KEY
	[4] = true, --HOUSE KEY
	[5] = true, --BUSINESS KEY
...
function getPlayerMaxCarryWeight(element)
	...
	if hasItem(element, 227) then weightCount = weightCount + 10 end -- military backpack
