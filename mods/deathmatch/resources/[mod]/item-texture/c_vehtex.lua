...
function vehTex_showGui(editVehicle)
	...
	local windowTitle = "Seznam textur pro vozidlo s VINem "..tostring(vehID)
	...
	gui.remove = guiCreateButton ( 10, height - 90, width - 20, 25, "Odstranit vybranou texturu", false, gui.window )
	gui.add = guiCreateButton ( 10, height - 60, width - 20, 25, "Přidat novou texturu", false, gui.window )
...
function vehTex_addGui()
	...
	gui.window2 = guiCreateWindow(634, 416, 456, 166, "Přidání nové textury na vozidlo", false)
	...
	gui.addCancel = guiCreateButton(16, 109, 199, 43, "Zrušit", false, gui.window2)
	gui.addApply = guiCreateButton(230, 109, 214, 43, "Potvrdit", false, gui.window2)
...
function vehTex_addGui_apply()
	...
	guiSetText(gui.addApply, "Prosím počkej...")
	...
	if (not texname or texname == "" or texname == " ") then
		vehTex_error("Nevybral jsi žádnou texturu pro předělání.")
		return false
	end
	if not isURLValid(texurl) then
		vehTex_error("Neplatná URL! Povolené typy souborů jsou .jpg a .png")
		return false
	end
	...
	if fileExists(path) then --file already exists, so we dont need to validate
		...
		guiSetText(gui.addApply, "Prosím počkej. Stahování...")
	end
end
function vehTex_fileValidationResult(editVehicle, texname, texurl, approved, msg)
	...
	if approved then
		...
		vehTex_error("Ověření souboru se nezdařilo! \n"..tostring(msg))
		return false		
	end
end
