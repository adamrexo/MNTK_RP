...
local maxFileSize = 1000000000 --50kb
...
function validateVehicleTexture(theVehicle, texName, url)
	fetchRemote(url, 5, function(str, errno, client)
			if str == 'ERROR' then
				...
				local text = "Adresa URL není dostupná. Zkontroluj, zdali jsi zadal správnou adresu URL a zda je adresa URL dostupná. (ERROR #"..tostring(errno)..")"
				...
				if filesize > maxFileSize then
					local text = "Velikost souboru ("..tostring(filesize).."b) přesahuje maximální povolenou velikost pro polepy na vozidla ("..tostring(maxFileSize).."b ("..tostring(maxFileSizeTxt).."))."
					triggerClientEvent(client, 'vehtex:fileValidationResult', resourceRoot, theVehicle, texName, url, false, text)
					fileDelete(path)
				else
