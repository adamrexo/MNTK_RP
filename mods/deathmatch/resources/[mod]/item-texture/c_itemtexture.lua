		...
		if fileExists(path) then
			...
			if data then --shader exist
				...
				if isPreview then
					...
					outputChatBox("Tvůj náhled skončí za 30 sekund, nebo /cancelpreview.", 255, 194, 14)
					textureName = texName 
					veh = element
				end	
			else
...
function validateFileFromURL(url)
	...
	if fileExists(path) then --file already exists, so we can simply check filesize
		...
		if size > maxFileSize then
			local text = "Velikost souboru přesahuje maximální povolenou velikost pro textury itemů ("..tostring(math.floor(maxFileSize/1000)).."kb)."
...
function godebug(cmd)
	if exports.integration:isPlayerScripter(localPlayer) then
		...
		outputChatBox("Item-texture debug nastaven na "..tostring(debugMode))
	end
end
...
function cancelPreview()
	if isTimer(previewTimer) then
		...
		textureName, veh, previewTimer = nil
		outputChatBox("Vypnul jsi náhled.", 255, 194, 14)
	else
		outputChatBox("Momentálně si nezobrazuješ náhled polepu vozidla.", 255, 194, 14)
	end
end	
