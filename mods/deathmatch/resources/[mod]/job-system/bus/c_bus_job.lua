...
function displayBusJob()
	...
	exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Přibližte se ke žluté značce na radaru a nastupte do autobusu, abyste mohli začít pracovat.")
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Již jste zahájili autobusovou trasu.")
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Sledujte autobusovou trasu, zastavte na autobusových zastávkách (červená značka).")
			else
				exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Musite byt v Bus ale v Coach pro zahájení trasy")
			end
		end
	else
		exports.hud:sendBottomNotification(localPlayer, "Oznámení", "Nejste řidič autobusu. Jděte na městský úřad a získejte tuto práci.")
	end
end
	...
	if thePlayer == getLocalPlayer() then
		...
		if vehicle and bus[getElementModel(vehicle)] then
			if curCpType == 3 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Řidič autobusu"}, {""}, {"Na zastávce chvíli počkejte, dokud značka nezmizí."}, }, false, false, false, 5)
			...
			exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Musite byt v Bus ale v Coach pro zahájení trasy.") -- Wrong car type.
		end
	end
end
...
function checkWaitAtStop(thePlayer)
	if thePlayer == getLocalPlayer() then
		if busStopTimer then
			exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Nečekal jsi na autobusové zastávce.")
...
function endOfTheLine()
	if busNextBlip then
		...
		setVehicleLocked(vehicle, false)
		exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Konečná zastavka. Pro opětovné zahájení trasy použijte /startbus.") --if line is finished
	end
end
			...
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 5)then
						...
						outputChatBox("Nemůžete si dovolit autobusové jízdné za $5.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("Za jízdu autobusem jste zaplatili $5.", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				exports.hud:sendBottomNotification(localPlayer, "Řidič autobusu", "Pro zahájení trasy autobusu použijte /startbus.")
			end
		end
	end
end
...
function startEnterBus(thePlayer, seat)
	if seat == 0 and bus[getElementModel(source)] then
		if getVehicleController(source) then -- if someone try to jack the driver stop him
			cancelEvent()
			if thePlayer == getLocalPlayer() then
				exports.hud:sendBottomNotification(localPlayer, "Oznámení", "Dveře řidiče jsou zamčené.")
			end
		else
