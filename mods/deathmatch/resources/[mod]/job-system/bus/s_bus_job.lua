...
function busAdNextStop(line, stop)
	...
	if(stop<#g_bus_routes[line].stops)then
		exports.hud:sendBottomNotification(source, "Rozhlas v autobusu", "Aktuální zastávka: [".. g_bus_routes[line].stops[stop] .. "]     Příští zastávka: [".. g_bus_routes[line].stops[stop+1] .. "] ")
	end
end
