...
function mechanicWindow(vehicle)
	...
	if (job==16) or (job==15) or (job==13) or exports.integration:isPlayerTrialAdmin(localPlayer, true) then
		...
		if not vehicle then
			...
			if not (wMechanic) then
				...
				if not isPedInVehicle( localPlayer ) and getPedSimplestTask(localPlayer) ~= "TASK_SIMPLE_CAR_DRIVE" then
					...
					-- Recolour
					if faction[13] or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or faction[getElementData(vehicle, "faction")] ) ) then
