			...
			if (targetPlayer) then
				...
				if commandName == "sjail" then
					if tonumber(minutes) then
						exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: " .. adminTitle .. " uvěznil " .. targetPlayerName .. " na " .. minutes .. " minut.")
						exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: Důvod: " .. reason)
						exports.logs:dbLog(thePlayer, 4, targetPlayer,commandName.." for "..minutes.." mins, reason: "..reason)
					else
						exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: " .. adminTitle .. " uvěznil " .. targetPlayerName .. " "..minutes..".")
						exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: Důvod: " .. reason)
					...
					for index, player in pairs( getElementsByType("player")) do
						if tonumber( getElementData( player, "punishment_notification_selector") ) ~= 1 or player == thePlayer or player == targetPlayer then
							if tonumber(minutes) then
								outputChatBox("[ADMIN-JAIL]: " .. adminTitle .. " uvěznil " .. targetPlayerName .. " na " .. minutes .. " minut.", player, 255, 0, 0)
								outputChatBox("[ADMIN-JAIL]: Důvod: " .. reason, player, 255, 0, 0)
							else
								outputChatBox("[ADMIN-JAIL]: " .. adminTitle .. " uvěznil " .. targetPlayerName .. " "..minutes..".", player, 255, 0, 0)
								outputChatBox("[ADMIN-JAIL]: Důvod: " .. reason, player, 255, 0, 0)
							end
						end
					end
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (who) or not (minutes) or not (...) or (minutes<1) then
			...
			if commandName == "sojail" then
				exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: " .. adminTitle .. " uvěznit " .. accountUsername .. " za " .. minutesString)
				exports.global:sendMessageToAdmins("[ADMIN-JAIL-SILENCED]: Důvod: " .. reason)
			else
				outputChatBox("[ADMIN-JAIL]: " .. adminTitle .. " uvěznit " .. accountUsername .. " za " .. minutesString, root, 255, 0, 0)
				outputChatBox("[ADMIN-JAIL]: Důvod: " .. reason, root, 255, 0, 0)
			end
			exports.logs:dbLog(thePlayer, 4, thePlayer,commandName.." "..accountUsername.." for "..minutes.." mins, reason: "..reason)
			exports.announcement:makePlayerNotification(accountID, "Byl jsi uvězněn administrátorem "..exports.global:getPlayerFullIdentity(thePlayer, 1, true)..".", "Důvod: "..reason)
		end
	end
end
	...
	if(isElement(jailedPlayer)) then
		...
		if (timeServed) then
			...
			if (timeLeft<=0) and not (getElementData(jailedPlayer, "pd.jailtime")) then
				...
				outputChatBox("Odseděl sis svůj čas, užij si svobodu!", jailedPlayer, 0, 255, 0)
		...
		for key, value in ipairs(players) do
			if getElementData(value, "adminjailed") then
				if tonumber(getElementData(value, "jailtime")) then
					...
					outputChatBox("[JAIL] Důvod: " .. tostring(getElementData(value, "jailreason")), thePlayer, 255, 194, 15)
				else
