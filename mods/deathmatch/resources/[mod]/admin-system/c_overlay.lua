...
local function updateGUI()
	if show then
		...
		local onduty = getElementData( localPlayer, "admin_level" ) <= 7 and "Momentálně jsi mimo službu " or ""
		if getElementData( localPlayer, "duty_admin" ) == 1 or getElementData( localPlayer, "duty_supporter" ) == 1 then
			onduty = "Momentálně jsi ve službě"
		else
			onduty = ""
		end

		if exports.integration:isPlayerSupporter(localPlayer) or exports.integration:isPlayerTrialAdmin(localPlayer) then
			textString = getAdminTitle( localPlayer ) .. " " .. onduty .. admstr .. ( (getElementData( localPlayer, "admin_level" ) <= 10 )and ( " | " .. ( openReports - handledReports ) .. " - volné reporty" .. reporttext .. " | " .. handledReports .. " - řešené reporty" .. ownreporttext .. " | " .. ckAmount .. " - CK Requesty |") or "" )
		else
			textString = getAdminTitle( localPlayer ) .. " | " .. ( openReports - handledReports ) .. " - volné reporty" .. reporttext .. " | " .. handledReports .. " - řešené reporty" .. ownreporttext .. " | "
		end
	end
end
...
function drawText ( )
	if show and exports.hud:isActive() then
		if ( getPedWeapon( localPlayer ) ~= 43 or not getPedControlState( localPlayer, "aim_weapon" ) ) then
			...
			dxDrawText( textString, 5, sy-19, sx, sy, tocolor ( 255, 255, 255, 255 ), 1, "default-bold")
		end
	end
end
