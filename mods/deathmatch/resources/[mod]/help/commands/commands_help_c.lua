...
function commandsHelp()
	...
	if (loggedIn == 1) then
		if (myCommandsWindow == nil) then
			...
			myCommandsWindow = guiCreateWindow (0.15, 0.15, 0.7, 0.7, "Dostupné príkazy", true)
			guiWindowSetSizable(myCommandsWindow, false)
			local tabPanel = guiCreateTabPanel (0, 0.1, 1, 1, true, myCommandsWindow)
			local tlBackButton = guiCreateButton(0.8, 0.04, 0.2, 0.06, "Zatvoriť", true, myCommandsWindow) -- close button
			...
			for _, levelcmds in pairs( commands ) do
				...
				guiGridListAddColumn (list, "Príkaz", 0.15)
				guiGridListAddColumn (list, "Použitie", 0.2)
				guiGridListAddColumn (list, "Vysvetlenie", 0.5)
				guiGridListAddColumn (list, "Príklad", 0.7)
