...
function giveBikeLicense(usingGC)
	if usingGC then
		...
		if success then
			...
			exports.hud:sendBottomNotification(source, "Dopravní inspektorát", "Could not take GCs from your account. Reason: "..reason.."." )
			return false
		end
	end
	...
	if getElementData(player, "owner") == -2 and getElementData(player, "faction") == -1 and getElementModel(player) == 468 then
		if getElementData(player,"license.bike") == 3 then
			if getElementData(player, "license.bike.cangetin") then
				exports.hud:sendBottomNotification(player, "Dopravní inspektorát", "Pomocí „J“ můžete nastartovat motor a /kickstand před jízdou." )
			else
