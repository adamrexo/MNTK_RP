...
function updateUnansweredReports()
	local info = {}
	table.insert(info, {string.upper("NEVYŘEŠENÉ REPORTY"), 255,2555,255,255,1,"sans" })
				...
				if staff then
					for k,v in ipairs(staff) do
						if string.find(adminTeams, v) and not seenReport[i] then
							table.insert(info, {abrv.." Report #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' nahlásil '" .. tostring(getPlayerName(reported)) .. "' v " .. timestring .. ". Rieši: " .. handler .. "", r, g, b})
							seenReport[i] = true
						end
					end
					count = count + 1
	...
	if count == 0 then
		table.insert(info, {"Všechny reporty jsou vyřešeny, dobrá práce."})
	else
		--
	end
