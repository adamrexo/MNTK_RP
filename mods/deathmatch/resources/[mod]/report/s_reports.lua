...
function toggleStatus(thePlayer, section, status)
	local newstatus
	if status == "Zakázáno" then
		newstatus = "Povoleno"
	end
	if status == "Povoleno" then
		newstatus = "Zakázáno"
	end
	exports.global:sendWrnToStaff(getPlayerName(thePlayer).." přepnul "..section.." na "..newstatus, "STATUS")
end
addEvent("toggleStatus", true)
addEventHandler("toggleStatus", getRootElement(), toggleStatus)

-- addEventHandler("onResourceStart", getResourceRootElement(getThisResource()), function()
-- 		if not getElementData(resourceRoot, "vehtheft") then setElementData(resourceRoot, "vehtheft", "Zakázáno") end
-- 		if not getElementData(resourceRoot, "propbreak") then setElementData(resourceRoot, "propbreak", "Zakázáno") end
-- 		if not getElementData(resourceRoot, "papforg") then setElementData(resourceRoot, "papforg", "Zakázáno") end

-- 		-- If less than 3 pd members, goes to On hold automaticly
-- 		setTimer(function()
-- 			local numPdMembers = #getPlayersInTeam(exports.factions:getTeamFromFactionID(1)) + #getPlayersInTeam(exports.factions:getTeamFromFactionID(1)) --PD and HP
-- 			if numPdMembers < 3 then
-- 				setElementData(resourceRoot, "vehtheft", "Zakázáno")
-- 				setElementData(resourceRoot, "propbreak", "Zakázáno")
-- 				setElementData(resourceRoot, "papforg", "Zakázáno")
-- 			end
	...
	for key, value in ipairs(getElementsByType("player")) do
		if exports.integration:isPlayerStaff(value) then
			if exports.integration:isPlayerTrialAdmin(value) then
				str = ":: "..gmduty.."/"..gm.." SUP :: " .. duty .."/".. online .." ADM ::"
	...
	for key , report in pairs(reports) do
		if report[8] ~= key then
			if isSupporterReport(report[7]) then
				...
				if showMessage then
					outputChatBox("#5268ff #FFFFFFTvůj report ID#"..report[8].." byl přesunut na ID#"..key.." protože report před tebou byl vyřešen.", report[1], 70, 200, 30,true)
				end
			else -- Admin report
				adminNotice = adminNotice.."#"..report[8]..", "
				gmNotice = gmNotice.."#"..report[8]..", "
				if showMessage then
					outputChatBox("#5268ff #FFFFFFTvůj report ID#"..report[8].." byl přesunut na ID#"..key.." protože report před tebou byl vyřešen.", report[1], 255, 195, 15,true)
				end
			end
	...
	if showMessage then
		if adminNotice ~= "" then
			...
			for key, value in ipairs(admins) do
				...
				if (adminduty==1) then
					outputChatBox("#5268ff #FFFFFFReporty s ID "..adminNotice.." byly posunuty výše.", value, 255, 195, 15,true)
				end
			end
		end
		if gmNotice ~= "" then
			...
			for key, value in ipairs(gms) do
				...
				if (gmDuty == 1) then
					outputChatBox("#5268ff #FFFFFFReporty s ID "..gmNotice.." byly posunuty výše.", value, 70, 200, 30,true)
				end
			end
		end
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			if reports[id] then
				...
				if staff then
					outputChatBox("["..abrv.." #" .. id .."] (" .. playerID .. ") " .. tostring(getPlayerName(reporter)) .. " nahlásil (" .. reportedID .. ") " .. tostring(getPlayerName(reported)) .. " v " .. timestring .. ".", thePlayer, r, g, b)
					outputChatBox("Důvod: " .. reason, thePlayer, r,g,b)
					...
					if (isElement(admin)) then
						...
						outputChatBox("[#" .. id .."] Tento report řeší " .. getPlayerName(admin) .. " ("..adminName..").", thePlayer, 70, 200, 30)
					else
				...
				outputChatBox("#5268ff #FFFFFFNesprávné ID reportu.", thePlayer, 255, 0, 0,true)
			end
		end
	end
end
...
function changeReportType(thePlayer, commandName, id, rID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		if not (id) or not (rID) then
			outputChatBox("SYNTAX: " .. commandName .. " [Report ID] [Report typ ID]", thePlayer, 255, 194, 15)
			outputChatBox("SYNTAX: REPORT TYPY:", thePlayer, 255, 194, 15)
			...
			if reportID > #reportTypes or reportID < 1 then
				outputChatBox("Error: Žiadny report typ nebol nájdený na toto ID.", thePlayer, 255, 0, 0)
				return
			end
			if reports[id] then
				local reporter = reports[id][1]
				...
				if oldReportType == reportID then
					outputChatBox("Error: Report už bol na tento typ odoslaný.", thePlayer, 255, 0, 0)
					return
				end
				...
				outputChatBox("#5268ff#FFFFFFTvůj report byl změněn z '"..oname.."'' na '"..name.."' administrátorem "..adminUser, reporter, 255, 126, 0,true)
				for k, v in ipairs(staff) do
					if string.find(auxiliaryTeams, v) then
						outputChatBox("#5268ff #FFFFFFReport ID#"..id.." - Byl změněn na typ "..name..".", thePlayer, 255, 126, 0,true)
						for key, value in pairs(players) do
							if getElementData(value, "loggedin") == 1 then
                                outputChatBox(" ["..abrv.." #" .. id .."] (" .. playerID .. ") " .. tostring(getPlayerName(reporter)) .. " nahlásil (" .. reportedID .. ") " .. tostring(getPlayerName(reported)) .. " v " .. timestring .. ".", value, r, g, b)--200, 240, 120)
                                outputChatBox("Důvod: " .. reason, value, r,g,b)
							end
						end
					else
						if isSupporterReport(oldReportType) then
							for key, value in pairs(GMs) do
								local gmDuty = getElementData(value, "duty_supporter")
								if (gmDuty == 1) then
									outputChatBox("#5268f #FFFFFFReport ID#"..id.." - Byl změněn z typu '"..oname.. "'' na '"..name.."'", value, 255, 126, 0,true)
								end
							end
							for key, value in pairs(admins) do
								local aDuty = getElementData(value, "duty_admin")
								if aDuty == 1 then
									outputChatBox("#5268ff #FFFFFFReport ID#"..id.." - Byl změněn z typu '"..oname.. "'' na '"..name.."'", value, 255, 126, 0,true)
								end
							end
							return
						else
	...
	if originalReportID then
		...
		if report and reports[report] then
			...
			if (isElement(theAdmin)) then
					outputChatBox("["..abrv.." #" .. report .."] Hráč " .. getPlayerName(source) .. " se odpojil.", theAdmin, 255, 126, 0)--200, 240, 120)
			else
							...
							for key, value in ipairs(getElementsByType("players")) do
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										outputChatBox("["..abrv.." #" .. report .."] Hráč " .. getPlayerName(source) .. " se odpojil.", value, 255, 126, 0)
								...
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										...
										if adminduty == 1 or gmduty == 1 then
											outputChatBox("["..abrv.." #" .. report .."] Hráč " .. getPlayerName(source) .. " se odpojil.", value, 255, 126, 0)
											alreadyTold[value] = true
											removeElementData(value, "reportNum")
										end
									end
								end
							end
			...
			removeElementData(source, "reportNum")
	...
	for i = 1, #reports do
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if not alreadyTold[value] then
									...
									outputChatBox("["..abrv.." #" .. i .."] Report je nepřiřazený (" .. adminName .. " se odpojil)", value, 255, 126, 0)
									alreadyTold[value] = true
									update = true
								end
							end
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if not alreadyTold[value] then
									...
									if adminduty == 1 or gmduty == 1 then
										local adminName = getElementData(source, "account:username")
										outputChatBox("["..abrv.." #" .. i .."]  Report je nepřiřazený (" .. adminName .. " se odpojil)", value, 255, 126, 0)--200, 240, 120)
										alreadyTold[value] = true
										update = true
									end
								end
		...
		elseif reports[i] and reports[i][2] == source then
					...
					if string.find(auxiliaryTeams, usergroup) then
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if not alreadyTold[value] then
									local adminName = getElementData(source, "account:username")
									outputChatBox("["..abrv.." #" .. i .."] Nahlásený hráč " .. getPlayerName(source) .. " se odpojil z hry.", value, 255, 126, 0)
									alreadyTold[value] = true
									update = true
								end
							end
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if not alreadyTold[value] then
									...
									local adminduty = getElementData(value, "duty_admin")
									if adminduty == 1 or gmduty == 1 then
										outputChatBox("["..abrv.." #" .. i .."] Nahlásený hráč " .. getPlayerName(source) .. " se odpojil z hry.", value, 255, 126, 0)--200, 240, 120)
										update = true
										alreadyTold[value] = true
									end
								end
			...
			if reporter ~= source then
				local adminName = getElementData(source, "account:username")
				outputChatBox("#5268ff #FFFFFFTvůj report "..abrv.."#" .. i .. " byl uzavřen (" .. adminName .. " se odpojil)", reporter, 255, 126, 0,true)--200, 240, 120)
		...
		for i = 1, #reports do
			if reports[i] then
				...
				if not staff then
					...
					outputChatBox("#5268ff#FFFFFFTvůj report byl automaticky uzavřen.", reports[i][1], 255, 0, 0,true)
					reports[i] = nil
					update = true
				end
			end
...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	if getElementData(reportedPlayer, "loggedin") ~= 1 then
		outputChatBox("#5268ff#FFFFFFHráč kterého nahlasujete není přihlášen.", source, 255, 0, 0,true)
		return
	end
	...
	exports.mysql:query_free('INSERT INTO `reports`(`User`, `Target`, `Type`, `ReportColumn`, `Obsah`) VALUES ("'.. getElementData(source, "account:username") ..' ('.. getPlayerName(source) ..')","'.. getElementData(reportedPlayer, "account:username") ..' ('.. getPlayerName(reportedPlayer) ..')", "'.. reportType ..'",0,"'.. exports.mysql:escape_string(reportedReason) ..'")')
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			for key, value in ipairs(GMs) do
				...
				if (gmDuty == 1) then
					nigger = nigger + 1
					outputChatBox("["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " požádal o pomoc.", value, r, g, b)
					outputChatBox("Otázka: " .. reportedReason, value, r,g,b)
			...
			if not skipadmin then
				...
				for key, value in ipairs(GMs) do
					...
					if (gmDuty == 1) then
						outputChatBox("["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " požádal o pomoc.", value, r, g, b)--200, 240, 120)
						outputChatBox("Otázka: " .. reportedReason, value, r,g,b)
						skipadmin = true
						-- if reason2 and #reason2 > 0 then
			...
			outputChatBox("#5268ff #FFFFFFDěkujeme za odeslání reportu. (Report ID: #" .. tostring(slot) .. ").", source, 70, 200, 30,true)

			outputChatBox("#5268ff #FFFFFFReport lze zrušit příkazem /er.", source, 70, 200, 30,true)
			...
			for key, value in ipairs(getElementsByType("player")) do
				if getElementData(value, "loggedin") == 1 then
					if string.find(getElementData(value, "forum_perms"), usergroup) then -- Opens up functionality to have reports ONLY go to leaders or only members
						outputChatBox("["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " nahlásil (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. " v " .. timestring .. ".", value, r, g, b)--200, 240, 120)
						outputChatBox("Důvod: " .. reportedReason, value, r,g,b)
					end
				end
			end
			outputChatBox("#5268ff #FFFFFFDěkujeme za odeslání Vašeho reportu. (Report ID: #" .. tostring(slot) .. ").", source, 200, 240, 120,true)
			if not getElementData( reportedPlayer, "maskaid" ) then
				outputChatBox("#5268ff #FFFFFFNahlásili jste hráče #DBDBDB(" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. ".", source, 237, 145, 33,true)
			else
				outputChatBox("#5268ff #FFFFFFNahlásili jste hráče #DBDBDBMaskovaný  (M:" .. getElementData( reportedPlayer, "maskaid" ) .. ").", source, 237, 145, 33,true)
			end
			outputChatBox("#5268ff#FFFFFFDůvod: "..reportedReason, source, 200, 240, 120,true)
			outputChatBox("#5268ff#FFFFFFReport lze zrušit příkazem /er.", source, 200, 240, 120,true)
			break
		else -- Admins
			local admins = exports.global:getAdmins()
			local count = 0
				...
				if not gmsTold then
					...
					for key, value in ipairs(GMs) do
						...
						if (gmDuty == 1) and getElementData(value, "report-system:subcribeToAdminReports") then
							outputChatBox("["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " nahlásil (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. " v " .. timestring .. ".", value, r, g, b)--200, 240, 120)
							outputChatBox("Důvod: " .. reportedReason, value)
							gmsTold = true
						end
					end
				end
			...
			for k, usergroup in ipairs(staff) do
				if string.find(auxiliaryTeams, usergroup) then
					for key, value in ipairs(getElementsByType("player")) do
						if getElementData(value, "loggedin") == 1 then
							if not alreadyTold[value] then
								outputChatBox("[#" .. id .. "] je stále nezodpovězen: (" .. playerID .. ") " .. tostring(getPlayerName(reportingPlayer)) .. " nahlásil (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. " v " .. timestring .. ".", value, 200, 240, 120)
								alreadyTold[value] = true
							end
						end
					end
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			if not (reports[id]) then
				outputChatBox("#5268ff #FFFFFFNesprávne ID reportu.", thePlayer, 255, 0, 0,true)
			else
				local reportHandler = reports[id][5]

				if (reportHandler) then

					outputChatBox("#5268ff #FFFFFFReport #" .. id .. " už řeší administrátor " .. getPlayerName(reportHandler) .. " ("..getElementData(reportHandler,"account:username")..")", thePlayer, 255, 0, 0,true)
				else
					local reportingPlayer = reports[id][1]
					local reportedPlayer = reports[id][2]
					...
					exports.mysql:query_free('INSERT INTO `reports`(`User`, `Target`, `Type`, `ReportColumn`, `Obsah`) VALUES ("'.. getElementData(thePlayer, "account:username") ..' ('.. getPlayerName(thePlayer) ..')","'.. getElementData(reportingPlayer, "account:username") ..' ('.. getPlayerName(reportingPlayer) ..')", -1, 2,"-1")')
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											outputChatBox("[#" .. id .. "] - "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..") označil report #" .. id .. " za neplatný.", value, r, g, b)
											alreadyTold[value] = false
										end
									end
								end
							else
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox("[#" .. id .. "] - "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..") označil report #" .. id .. " za neplatný.", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = false
											end
										end
									end
					...
					outputChatBox("#5268ff #FFFFFFVáš report (#" .. id .. ") byl označen jako neplatný administrátorem "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..").", reportingPlayer, r, g, b,true)--200, 240, 120)
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					outputChatBox("#5268ff #FFFFFFReport #" .. id .. " už řeší administrátor " .. getPlayerName(reportHandler) .. ".", thePlayer, 255, 0, 0,true)
				else
					...
					if reportingPlayer == thePlayer and not exports.integration:isPlayerScripter(thePlayer) then
						outputChatBox("#5268ff #FFFFFFNemůžeš přijmout svůj report.",thePlayer, 255,0,0,true)
						return false
					--[[
					...
					exports.mysql:query_free('INSERT INTO `reports`(`User`, `Target`, `Type`, `ReportColumn`, `Obsah`) VALUES ("'.. getElementData(thePlayer, "account:username") ..' ('.. getPlayerName(thePlayer) ..')","'.. getElementData(reportingPlayer, "account:username") ..' ('.. getPlayerName(reportingPlayer) ..')", -1, 1,"-1")')
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") přijal report #" .. id .. "", value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") přijal report #" .. id .. "", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
					...
					outputChatBox("#5268ff #FFFFFF"..adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") přijal Váš report (#" .. id .. ") v "..timestring..".", reportingPlayer, 255,126, 0,true)--200, 240, 120)
					triggerClientEvent ( reportingPlayer, "playNudgeSound", reportingPlayer)

					outputChatBox("#5268ff[REPORT] #FFFFFFPřijal jsi report #" .. id .. ". Kontaktuj hráče #DBDBDB(" .. playerID .. ") " .. getPlayerName(reportingPlayer) .. "#FFFFFF.", thePlayer, 255,255,255,true)--200, 240, 120)
		...
		if not (...) then
			...
			if targetAdmin then
				...
				elseif (reports[id][5] ~= thePlayer) and not (exports.integration:isPlayerAdmin(thePlayer)) then
					outputChatBox("#5268ff #FFFFFFToto není tvoj report.", thePlayer, 255, 0, 0,true)
				else
					local reportingPlayer = reports[id][1]
					local reportedPlayer = reports[id][2]
					...
					outputChatBox("[" .. timestring .. "] " .. getPlayerName(thePlayer) .. " odevzdal report administrátorovi ".. getPlayerName(targetAdmin) .." (#" .. id .. "), PProsím čekejte dokud vás nekontaktuje.", reportingPlayer, 200, 240, 120)
					outputChatBox(getPlayerName(thePlayer) .. " ti odevzdal report #" .. id .. ". Prosím kontaktuj hráče ( (" .. playerID .. ") " .. getPlayerName(reportingPlayer) .. ").", targetAdmin, 200, 240, 120)
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											outputChatBox("[#" .. id .. "] - " .. getPlayerName(thePlayer) .. " odevzdal report #" .. id .. " administrátorovi  ".. getPlayerName(targetAdmin) , value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox("[#" .. id .. "] - " .. getPlayerName(thePlayer) .. " odevzdal report #" .. id .. " administrátorovi  ".. getPlayerName(targetAdmin) , value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				outputChatBox("#5268ff #FFFFFFToto není tvůj report.", thePlayer, 255, 0, 0,true)
			else
				local reporter = reports[id][1]
				local reported = reports[id][2]
				local reason = reports[id][3]
				...
				if (isElement(reporter)) then
					...
					outputChatBox("#5268ff #FFFFFF"..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") uzavřel tvůj report.", reporter, r, g, b,true)
					--triggerClientEvent(reporter, "feedback:form", thePlayer) -- Staff feedback / Maxime / 2015.1.29
				end

				exports.mysql:query_free('INSERT INTO `reports`(`User`, `Target`, `Type`, `ReportColumn`, `Obsah`) VALUES ("'.. getElementData(thePlayer, "account:username") ..' ('.. getPlayerName(thePlayer) ..')","'.. getElementData(reporter, "account:username") ..' ('.. getPlayerName(reporter) ..')", -1, 4,"-1")')
					...
					for k, usergroup in ipairs(staff) do
						if string.find(auxiliaryTeams, usergroup) then
							for key, value in ipairs(getElementsByType("player")) do
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") uzavřel report #" .. id .. "", value, r, g, b)
										alreadyTold[value] = true
									end
								end
							end
							...
							for key, value in ipairs(getElementsByType("player")) do
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										...
										local gmduty = getElementData(value, "duty_supporter")
										if (adminduty==1) or (gmduty==1) then
											outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") uzavřel report #" .. id .. "", value, r, g, b)--200, 240, 120)
											alreadyTold[value] = true
										end
									end
								end
...
function closeAllReports(thePlayer)
	if exports.integration:isPlayerStaff(thePlayer) then
		...
		if count == 0 then
			outputChatBox("#5268ff #FFFFFFŽádný report nebyl uzavřen.", thePlayer, 255, 126, 0,true)--255, 194, 15)
		else
			outputChatBox("#5268ff #FFFFFFUzavřel jsi "..count.." reportů.", thePlayer, 255, 126, 0,true)--255, 194, 15)
		end
	end
end
		...
		if not (id) then
			...
			if (reports[id] == nil) then
				...
				if (reports[id][5] ~= thePlayer) then
					outputChatBox("#5268ff #FFFFFFTento report neřešíš.", thePlayer, 255, 0,0,true)
				else
					...
					if (isElement(reporter)) then
						...
						exports.anticheat:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
						outputChatBox("#5268ff #FFFFFF"..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") uvolnil váš report.", reporter, r, g, b,true)
					end
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") přenechal report #" .. id .. "", value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox("["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") přenechal report #" .. id .. "", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
...
function endReport(thePlayer, commandName)
	...
	if not adminreport or not report then
		outputChatBox("#5268ff #FFFFFFNemáš podán žádný report.", thePlayer, 255, 0, 0,true)
		...
		exports.mysql:query_free('INSERT INTO `reports`(`User`, `Target`, `Type`, `ReportColumn`, `Obsah`) VALUES ("'.. getElementData(thePlayer, "account:username") ..' ('.. getPlayerName(thePlayer) ..')","-1", -1, 3,"-1")')

-- [" .. timestring .. "]
		outputChatBox("#5268f #FFFFFFUzavřel jsi svůj report ID#"..report, thePlayer, 200, 240, 120,true)
		local otherAccountID = nil
		if (isElement(reportHandler)) then
			outputChatBox(getPlayerName(thePlayer) .. " uzatvoril report (#" .. report .. ").", reportHandler, 255, 126, 0)--200, 240, 120)
...
function showUnansweredReports(thePlayer)
	if exports.integration:isPlayerStaff(thePlayer) then
		if showTopRightReportBox(thePlayer) then
			...
			outputChatBox("#5268ff>>>> #FFFFFFNezodpovězené reporty #5268ff<<<<", thePlayer, 0, 255, 15,true)
			--reports = sortReportsByTime(reports)
			local count = 0
			local seenReport = { }
			for i = 1, #reports do
						...
						if staff then
							for k,v in ipairs(staff) do
								if not seenReport[i] then
									outputChatBox("Report "..abrv.."#" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' nahlásil '" .. tostring(getPlayerName(reported)) .. "' v " .. timestring .. ".", thePlayer, r, g, b)
									count = count + 1
									seenReport[i] = true
								end
							end
			...
			if count == 0 then
				outputChatBox("#FFFFFFVšechny reporty jsou vyřešeny, #5268ffdobrá práce#FFFFFF.", thePlayer, 255, 194, 15,true)
			else
				outputChatBox("#5268ff>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<", thePlayer, 0, 255, 15,true)
			--	outputChatBox("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", thePlayer, 0, 255, 15)
				outputChatBox("#FFFFFFNapište /ri [ID] pro zjištění více informací o reportu.", thePlayer, 255, 194, 15,true)
			end
		end
	end
end
...
function showReports(thePlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if showTopRightReportBox(thePlayer) then
			...
			outputChatBox("#5268ff>>>> #FFFFFFReporty #5268ff<<<<", thePlayer, 255, 194, 15,true)
			--reports = sortReportsByTime(reports)
			local count = 0
			for i = 1, #reports do
				local report = reports[i]
					...
					if staff then
						for k,v in ipairs(staff) do
							if exports.integration:isPlayerTrialAdmin(thePlayer) and not seenReport[i] then
								outputChatBox("Report "..abrv.."#" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' nahlásil '" .. tostring(getPlayerName(reported)) .. "' v " .. timestring .. ". Řeší: " .. handler .. "", thePlayer, r, g, b)
								count = count + 1
								seenReport[i] = true
							end
						end
			...
			if count == 0 then
				outputChatBox("#FFFFFFŽádné reporty nebyly řešeny.", thePlayer, 255, 194, 15,true)
			else
				--outputChatBox("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", thePlayer, 255, 221, 117)
				--outputChatBox("Type /ri [id] to obtain more information about the report.", thePlayer, 255, 194, 15)
			end
...
function updateStaffReportCount(thePlayer)
	...
	if adminreports_saved >= reportsToAward then
		...
		exports.achievement:awardPlayer(thePlayer, false, "Vyřešených "..reportsToAward.." reportů.", gcToAward)
		exports.global:sendWrnToStaff(exports.global:getPlayerFullIdentity(thePlayer).." získal "..gcToAward.." kreditů za vyřešení "..reportsToAward.." reportů.", "REPORTY")
	else
...
function getSavedReports(thePlayer)
	local adminreports_saved = getElementData(thePlayer, "adminreports_saved") or 0
	outputChatBox("#5268f #FFFFFFMáš "..adminreports_saved.." vyřešených reportů. "..reportsToAward-adminreports_saved.." reportů chybí k odměně.", thePlayer, 255, 126, 0,true)
end
addCommandHandler("getsavedreports", getSavedReports)
addCommandHandler("mojereporty", getSavedReports)
addCommandHandler("reporty", getSavedReports)
...
-- 	outputChatBox("#5268ff#FFFFFFNastavil jsi "..reports..".", thePlayer, 255, 126, 0,true)
-- end
-- addCommandHandler("setsavedreports", setSavedReports)


-- addCommandHandler( "fixReport", function (thePlayer)
-- 	removeElementData(thePlayer, "reportNum")
-- end, false, false )