...
function startFactionSelect(fTable, selectiontype)
    ...
    factionSelectGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Vyberte frakci, kterou chcete zobrazit", false)
    ...
    factionSelectGUI.button[1] = guiCreateButton(13, 64, 111, 42, "Zrušit", false, factionSelectGUI.window[1])
    guiSetProperty(factionSelectGUI.button[1], "NormalTextColour", "FFAAAAAA")
    factionSelectGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Vyberte frakci, kterou chcete zobrazit", false, factionSelectGUI.window[1])
    factionSelectGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Potvrdit", false, factionSelectGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionSelectGUI.button[2], function()
    	...
    	if name ~= "Vyberte frakci, kterou chcete zobrazit" then
    		...
    		outputChatBox("Vyberte prosím frakci.", 255, 0, 0)
    	end
    end, false)
...
function fillBusiness(name)
	if wBank and isElement(wBank) then
		...
		if faction then
			...
			guiSetText(lBalanceB, "Zůstatek: $" .. exports.global:formatMoney(gfactionBalance))
		end
	end
end
...
function showBankUI(fTable, depositable, limit, withdrawable, ped)
	if not (wBank) then
		...
		wBank = guiCreateWindow(x, y, width, height, "Banka Los Santos | "..version.."", false)
		...
		tabPersonal = guiCreateTab("Osobní bankovnictví", tabPanel)
		tabPersonalTransactions = guiCreateTab("Osobní bankovnictví", tabPanel)

		local hoursplayed = getElementData(localPlayer, "hoursplayed") or 0
		if (exports.global:countTable(fTable) > 0) then
			tabBusiness = guiCreateTab("Obchodní bankovnictví", tabPanel)

			lBalanceB = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Zůstatek: $", true, tabBusiness)
			guiSetFont(lBalanceB, "default-bold-small")

			if (withdrawable) then
			-- WITHDRAWAL BUSINESS
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Vybrat:", true, tabBusiness)
				...
				bWithdrawB = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Vybrat", true, tabBusiness)
			...
			if (depositable) then
				-- DEPOSIT BUSINESS
				lDepositB = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Vklad:", true, tabBusiness)
				...
				bDepositB = guiCreateButton(0.44, 0.23, 0.2, 0.075, "Vložit", true, tabBusiness)
			...
			if hoursplayed >= 12 then
				-- TRANSFER BUSINESS
				lTransferB = guiCreateLabel(0.1, 0.45, 0.2, 0.05, "Převod:", true, tabBusiness)
				...
				bTransferB = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Převést", true, tabBusiness)
				addEventHandler("onClientGUIClick", bTransferB, transferMoneyBusiness, false)

				eTransferB = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Číslo bankovního účtu>", true, tabBusiness)
				addEventHandler("onClientGUIClick", eTransferB, function()
					if guiGetText(eTransferB) == "<Číslo bankovního účtu>" then
						guiSetText(eTransferB, "")
					end
				end, false)

				lTransferBReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Důvod:", true, tabBusiness)
				guiSetFont(lTransferBReason, "default-bold-small")

				tTransferBReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<K čemu tato transakce slouží?>", true, tabBusiness)
				addEventHandler("onClientGUIClick", tTransferBReason, function()
					if guiGetText(tTransferBReason) == "<K čemu tato transakce slouží?>" then
		...
		bClose = guiCreateButton(0.75, 0.91, 0.2, 0.1, "Zavřít", true, wBank)
		...
		--shit
		lBalance = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Zůstatek: $" .. exports.global:formatMoney(balance), true, tabPersonal)
		guiSetFont(lBalance, "default-bold-small")

		if withdrawable then
			-- WITHDRAWAL PERSONAL
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Vybrat:", true, tabPersonal)
			...
			bWithdrawP = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Vybrat", true, tabPersonal)
		...
		if (depositable) then
			-- DEPOSIT PERSONAL
			lDepositP = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Vklad:", true, tabPersonal)
			...
			bDepositP = guiCreateButton(0.44, 0.23, 0.2, 0.075, "Vklad", true, tabPersonal)
		...
		if hoursplayed >= 12 then
			-- TRANSFER PERSONAL
			lTransferP = guiCreateLabel(0.1, 0.45, 0.2, 0.05, "Převod:", true, tabPersonal)
			...
			bTransferP = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Převest: ", true, tabPersonal)
			addEventHandler("onClientGUIClick", bTransferP, transferMoneyPersonal, false)

			eTransferP = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Jméno hráče/frakce>", true, tabPersonal)
			addEventHandler("onClientGUIClick", eTransferP, function()
				if guiGetText(eTransferP) == "<Jméno hráče/frakce>" then
					guiSetText(eTransferP, "")
				end
			end, false)

			lTransferPReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Důvod:", true, tabPersonal)
			guiSetFont(lTransferPReason, "default-bold-small")

			tTransferPReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<K čemu tato transakce slouží?>", true, tabPersonal)
			addEventHandler("onClientGUIClick", tTransferPReason, function()
				if guiGetText(tTransferPReason) == "<K čemu tato transakce slouží??>" then
...
function transferMoneyPersonal(button)
	if (button=="left") then
		...
		if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
			outputChatBox("Zadejte prosím kladnou částku!", 255, 0, 0)
		elseif (amount>money) then
			outputChatBox("Nemáte dostatek finančních prostředků.", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Zadejte prosím důvod převodu!", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Zadejte prosím celé jméno postavy příjemce!", 255, 0, 0)
		else
...
function withdrawMoneyBusiness(button)
	if (button=="left") then
		...
		elseif (amount>gfactionBalance) then
			outputChatBox("Nemáte dostatek finančních prostředků.", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Tento bankomat umožňuje pouze výběr $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
			setElementData( lastUsedATM, "withdrawn", oldamount + amount, false )
			setTimer(
				function( atm, amount )
...
function checkDataChange(dn)
	if wBank then
		if dn == "bankmoney" and source == localPlayer then
			guiSetText(lBalance, "Zůstatek: $" .. exports.global:formatMoney(getElementData(source, "bankmoney")))
