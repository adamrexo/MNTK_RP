...
function neonTable.func.createGuiBox()
	...
	local left = scx / 10 
	local top = scy / 10
	neonTable.gui.root = dgsCreateWindow(left, top, windowWidth, windowHeight, "Neon v0.1 by Ren712", false)
	dgsWindowSetSizable(neonTable.gui.root, false)
	
    neonTable.gui.name = dgsCreateLabel(220, 170, 201, 21, "No image chosen.", false, neonTable.gui.root)
	dgsLabelSetHorizontalAlign(neonTable.gui.name, "center", false)
	dgsLabelSetVerticalAlign(neonTable.gui.name, "center")
	
	neonTable.gui.imageList = dgsCreateGridList(10, 26, 201, 257, false, neonTable.gui.root)				
	if neonTable.func.imageList then
		addEventHandler("onDgsMouseClickUp", neonTable.gui.imageList, neonTable.func.imageList, false)
	end
	
	dgsSetText(neonTable.gui.name, "No image chosen.")

	neonTable.gui.toggleColor = dgsCreateButton(220, 195, 101, 21, "Color", false, neonTable.gui.root)
	if neonTable.func.toggleColor then
		addEventHandler("onDgsMouseClickUp", neonTable.gui.toggleColor, neonTable.func.toggleColor, false)
	end
	
	neonTable.gui.apply = dgsCreateButton(220, 215, 101, 21, "Apply", false, neonTable.gui.root)
	if neonTable.func.apply then
		addEventHandler("onDgsMouseClickUp", neonTable.gui.apply, neonTable.func.apply, false)
	end
	
	neonTable.gui.close = dgsCreateButton(320, 215, 101, 21, "Close", false, neonTable.gui.root)
	if neonTable.func.close then
		addEventHandler("onDgsMouseClickUp", neonTable.gui.close, neonTable.func.close, false)
	end
	
	neonTable.gui.scrollX =  dgsCreateScrollBar(290,239,130,21,true, false, neonTable.gui.root)
	dgsScrollBarSetScrollPosition ( neonTable.gui.scrollX, 50 )
	neonTable.gui.scrollXLabel = dgsCreateLabel(220, 239, 101, 21, "SizeX: 0.5", false, neonTable.gui.root)
	if neonTable.func.scrollX then
		addEventHandler("onDgsElementScroll", neonTable.gui.scrollX, neonTable.func.scrollX, false)
	end	
	
	neonTable.gui.scrollY =  dgsCreateScrollBar(290,263,130,21,true, false, neonTable.gui.root)
	dgsScrollBarSetScrollPosition ( neonTable.gui.scrollY, 50 )
	neonTable.gui.scrollYLabel = dgsCreateLabel(220, 263, 101, 21, "SizeY: 0.5", false, neonTable.gui.root)
	if neonTable.func.scrollY then
		addEventHandler("onDgsElementScroll", neonTable.gui.scrollY, neonTable.func.scrollY, false)
	end	
	
	neonTable.gui.clear = dgsCreateButton(320, 195, 101, 21, "Clear", false, neonTable.gui.root)
	if neonTable.func.clear then
		addEventHandler("onDgsMouseClickUp", neonTable.gui.clear, neonTable.func.clear, false)
	end
end
...
function neonTable.func.toggleMeme()
	if neonTable.isGuiVisible then 
		...
		dgsSetVisible(neonTable.gui.root, false)
		return 
	else
		showCursor(true)
		neonTable.isGuiVisible = true
		dgsSetVisible(neonTable.gui.root, true)
	end
end
...
function neonTable.func.imageList()
	local chosenListItem = dgsGridListGetSelectedItem (neonTable.gui.imageList)
	local chosenListText = dgsGridListGetItemText (neonTable.gui.imageList, chosenListItem, 1)
	if chosenListText == "" then 
		...
		dgsSetText(neonTable.gui.scrollXLabel, "SizeX: "..neonTable.image.size[1])
		dgsSetText(neonTable.gui.scrollYLabel, "SizeY: "..neonTable.image.size[2])
		dgsScrollBarSetScrollPosition ( neonTable.gui.scrollX, math.abs(neonTable.image.size[1] * 100) )
		dgsScrollBarSetScrollPosition ( neonTable.gui.scrollY, math.abs(neonTable.image.size[2] * 100) )
		dgsSetText(neonTable.gui.name, neonTable.image.name)
		dgsGridListSetSelectedItem(neonTable.gui.imageList, tonumber(neonTable.image.id) - 1 ,  1)
	else
		local chosenColor = getElementData(localPlayer, "neonColor")
		local chosenSize = getElementData(localPlayer, "neonSize")
...
function neonTable.func.scrollX(scrolled)
	...
	local scrollPosition = dgsScrollBarGetScrollPosition(scrolled)
...
addEventHandler("onClientHUDRender", root, function()  
	...
	local posX, posY = dgsGetPosition ( neonTable.gui.root, false )
...
addEventHandler("onClientPreRender", root, function()
	...
	for index,thisPed in ipairs(getElementsByType("player", root)) do	
...
addEventHandler("onClientResourceStart", resourceRoot, function()
	...
	dgsSetVisible (neonTable.gui.root, false)
	local imageColumn = dgsGridListAddColumn ( neonTable.gui.imageList,"Choose your image",0.85)
	...
	for i,name in ipairs(children) do 
		...
		local imageRow = dgsGridListAddRow(neonTable.gui.imageList)
		...
		dgsGridListSetItemText(neonTable.gui.imageList, imageRow, imageColumn, imageName, false, false )
	end
	xmlUnloadFile(meta)
	...
	if neonTable.image.id and neonTable.image.color and neonTable.image.size then
		...
		dgsGridListSetSelectedItem(neonTable.gui.imageList, 0, 1)
		...
		dgsSetText(neonTable.gui.scrollYLabel, "SizeY: "..neonTable.image.size[2])
		dgsScrollBarSetScrollPosition ( neonTable.gui.scrollX, math.abs(neonTable.image.size[1] * 100) )
		dgsScrollBarSetScrollPosition ( neonTable.gui.scrollY, math.abs(neonTable.image.size[1] * 100) )	
