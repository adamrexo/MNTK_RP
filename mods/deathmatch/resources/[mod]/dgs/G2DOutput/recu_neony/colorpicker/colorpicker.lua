...
colorPicker = {
	...
	constructor = function( info )
		...
		colorPicker.GUI.selectWindow = dgsCreateWindow(screenW - info.selectWindow.width, (screenH - info.selectWindow.height) / 2,
		                                        info.selectWindow.width, info.selectWindow.height, "Pick a color (click outside window to close)", false)
		dgsSetVisible(colorPicker.GUI.selectWindow, false)
		dgsWindowSetSizable(colorPicker.GUI.selectWindow, false)

		colorPicker.GUI.palette = dgsCreateImage(colorPicker.selectWindow.paletteX, colorPicker.selectWindow.paletteY,
		                                        256, 256, "colorpicker/palette.png", false, colorPicker.GUI.selectWindow)
		colorPicker.GUI.alphaBar = dgsCreateImage(colorPicker.selectWindow.paletteX + 255 + colorPicker.selectWindow.alphaOffset, colorPicker.selectWindow.paletteY,
	...
	forceNaturalAndRange = function()
		local inputText = dgsGetText( source )
		if not tonumber( inputText ) then
			local changedText = string.gsub( inputText, "[^%d]", "" )
			if changedText ~= inputText then
				dgsSetText(source, changedText)
			end
		end
		
		local inputNumber = tonumber(dgsGetText( source ))
		if inputNumber then
			...
			if clampedNumber ~= inputNumber then
				dgsSetText(source, tostring(clampedNumber))
			end
		end
	end,
	...
	selectionManualInputRGB = function()
		if not colorPicker.avoidRecursion then
			colorPicker.avoidRecursion = true
			local r, g, b, a = 	tonumber(dgsGetText(colorPicker.children.R)),
								tonumber(dgsGetText(colorPicker.children.G)),
								tonumber(dgsGetText(colorPicker.children.B)),
								tonumber(dgsGetText(colorPicker.children.A))
	...
	selectionManualInputHSL = function()
		if not colorPicker.avoidRecursion then
			colorPicker.avoidRecursion = true
			local h, s, l = tonumber(dgsGetText(colorPicker.children.H)),
			                tonumber(dgsGetText(colorPicker.children.S)),
							tonumber(dgsGetText(colorPicker.children.L))
...
--This is where the color updates every time you click the palette or presets!
	...
	openSelect = function( currentColor )
		...
		dgsSetVisible(colorPicker.GUI.selectWindow, true)
		dgsBringToFront(colorPicker.GUI.selectWindow)
	...
	updateSelectedValue = function()
		if not dgsGetVisible(colorPicker.GUI.selectWindow) then return end

		local r, g, b, a

		-- Check for color changes
		local wx, wy = dgsGetPosition(colorPicker.GUI.selectWindow, false)
...
--Wont work until I get the actual mouse working
	...
	pickColor = function( button, state, cursorX, cursorY )
		...
		local wx, wy = dgsGetPosition(colorPicker.GUI.selectWindow, false)
		local ww, wh = dgsGetSize(colorPicker.GUI.selectWindow, false)
