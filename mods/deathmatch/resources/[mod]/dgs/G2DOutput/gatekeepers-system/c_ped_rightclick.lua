	...
	if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") and (sent==false) and (element~=getLocalPlayer()) then
		...
		if (gatekeeper) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				wPedRightClick = dgsCreateWindow(ax-w/2, ay-h/2, w, h , "NPC", false)
				local l1 = dgsCreateLabel(0, 0.08, 1, 0.25, pedName, true, wPedRightClick)
				dgsLabelSetHorizontalAlign(l1, "center")
				bTalkToPed = dgsCreateButton(0.05, 0.3, 0.87, 0.25, "Mluvit", true, wPedRightClick)
				addEventHandler("onDgsMouseClickUp", bTalkToPed,  function (button, state)
					if(button == "left" and state == "up") then
						...
						elseif (ped=="Robert Dunston") then --HP release / Maxime
							triggerServerEvent("tow:openReleaseGUI", localPlayer, ped)
						elseif (ped=="Pošťák") then
                            triggerEvent("postak:okno", getLocalPlayer())
                        elseif (ped=="Zákazník čekající na dopis") then
                            triggerEvent("postak:odovzdat", getLocalPlayer())
						elseif (ped=="Melina Dupont") then -- Custom clothes NPC at Dupont HQ.
							...
							exports.hud:sendBottomNotification(getLocalPlayer(),(ped or "NPC").." povídá:", "        'Ahoj'")
						end
					end
				end, false)

				bClosePedMenu = dgsCreateButton(0.05, 0.6, 0.87, 0.25, "Zavřít", true, wPedRightClick)
