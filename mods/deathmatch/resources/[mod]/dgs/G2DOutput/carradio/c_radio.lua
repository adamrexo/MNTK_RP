...
function showMP3GUI()
	destroyMP3Window()
	mp3Window = dgsCreateWindow(mp3_x, mp3_y, mp3_w, mp3_h, "MP3", false)

	local grid = dgsCreateGridList(10, 20, 180, 205, false, mp3Window)
	local col = dgsGridListAddColumn(grid, "Station", 0.85)
	for k, v in ipairs(streams) do
		if k ~= 0 then
			local row = dgsGridListAddRow(grid)
			dgsGridListSetItemText(grid, row, col, v[1], false, false)
			dgsGridListSetItemData(grid, row, col, tostring(k))
		end
	end

	volSlider = dgsCreateScrollBar(10, 235, 180, 19, true, false, mp3Window)

	if mp3Sound then 
		local setBar = (getSoundVolume(mp3Sound) * 100)
		dgsScrollBarSetScrollPosition(volSlider, setBar)
	else
		dgsScrollBarSetScrollPosition(volSlider, 100)
	end

	addEventHandler("onDgsElementScroll", volSlider, 
		function()
			if volSlider then 
				local sliderPos = dgsScrollBarGetScrollPosition(volSlider)
	...
	addEventHandler("onDgsMouseClickUp", grid,
		function()
			local item = dgsGridListGetSelectedItem(grid)
			if item ~= -1 then
				local station = tonumber(dgsGridListGetItemData(grid, item, col))
	...
	local off = dgsCreateButton(10, height - 35, 85, 25, "Turn Off", false, mp3Window)
	addEventHandler("onDgsMouseClickUp", off,
		function()
			if mp3Sound then
				stopSound(mp3Sound)
				mp3Sound = nil
	...
	local close = dgsCreateButton(105, height - 35, 85, 25, "Close", false, mp3Window)
	addEventHandler("onDgsMouseClickUp", close, destroyMP3Window, false)
end
